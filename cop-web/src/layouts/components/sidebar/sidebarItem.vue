<template>
  <div class='menu-wrapper' v-if="!menuItem.hidden">
    <template v-if="menuItem.children">
      <MenuItem v-if="hasOneChild(menuItem.children, menuItem)" :path="singleChild.path" :meta="singleChild.meta" :default-icon="defaultIcon">
      </MenuItem>
      <el-submenu v-else :index="currPath">
        ...
        <template v-for="child in menuItem.children">
          ...
          <MenuItem v-else-if="!child.hidden" :path="resolvePath(currPath, child.path)" :meta="child.meta" :key="child.path" :default-icon="defaultIcon">
          </MenuItem>
        </template>
      </el-submenu>
    </template>
    <MenuItem v-else :path="currPath" :meta="menuItem.meta">
    </MenuItem>
  </div>
</template>
