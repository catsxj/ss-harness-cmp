<template>
  <div>
    <AdvanceTable title="历史对比列表" :data="list" :searchConfigs="searchConfigs" :params="params" :total="total" :columns="historyColumns" :get-list="getList" :loading="loading">
    ...
    <build-history-compare v-if="buildVisible" @goBack="buildGoBack" :type="'3'"></build-history-compare>
...
import { getHistoryCompareList, removeCrossCompare, operateBackups } from 'services/task/compare'
import { historyColumns, searchConfigs } from './config'
export default {
  components: {
    BuildHistoryCompare,
    ViewCompare
  ...
  methods: {
    ...
    getList() {
      getHistoryCompareList(this.params).then((data) => {
