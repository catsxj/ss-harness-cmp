...
export default {
  ...
  methods: {
    getOsList (flag) {
      const { location: { vendorId } } = this.addData
      conditionImage({
    ...
    getImage (flag) {
      const { location: { vendorId }, configs: { osCategory, osVersion } } = this.addData;
      ...
      conditionImage({
        ...
        if (data.success) {
          ...
          if (flag && this.addData.location.vendorType == 'VMWARE' && (this.addData.configs.networkCardConfigs.length == 0 || this.addData.configs.networkCardConfigs[0].networkCardId != data.data.networkCards[0].id)) {
            this.$set(this.addData.configs, 'networkCardConfigs', data.data.networkCards.map(item => {
              return {
