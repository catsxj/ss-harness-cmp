    ...
    <el-col :span="8">
      <el-form-item label="镜像：" required>
        <el-select filterable v-model="imageName" @change="setImageId">
          <el-option v-for="(value, name, index) in imageList" :key="index"
                     :label="name" :value="name"></el-option>
        </el-select>
      </el-form-item>
    </el-col>
    <el-col :span="6">
      <basic-form-item label-width="10px" label="" prop="imageId" validate="required" required-message="该字段为必填字段">
        <el-select filterable v-model="addData.imageId" @change="setOsCategory">
          <el-option v-for="(item, index) in imageList[imageName]" :key="index"
                     :label="item.name + '(大小：' + item.size + 'GB)'" :value="item.id"></el-option>
        </el-select>
      </basic-form-item>
    </el-col>
  </el-col>
