    ...
    <div class="fix-height" v-show="activeStep === 1">
      ...
      <el-row>
        <basic-form :model="addData" ref="addForms" label-position="left" :disabled="!!itemData && !this.$route.query.id">
          <RegionItem :code="code" :add-data="addData" :vendor-type="vendorType" @vendorId="$emit('vendorId')" @backZone="$emit('backZone')" :payList="payList" :item-data="itemData" :disabled="disabled"></RegionItem>
        </basic-form>
...
export default {
  components: { RegionItem, ConfirmOrder, user },
  props: {
    addData: {
      type: Object,
      required: true
