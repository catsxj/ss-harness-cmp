...
<template>
  <common-wrapper code="compute" :add-data="addData" @vendorId="getList" @backZone="getSubnet" ref="common" :elements="elements" :get-params="getParams" :modify-element="modifyElement" :item-data="retention" :disabled="disabled">
    ...
    <images :add-data="addData.configs" :vendor-id="addData.location.vendorId" :location="addData.location" vendorType="HUAWEI" :item-data="retention"></images>
    <el-col :span="6">
      <basic-form-item label="系统盘：" prop="configs.rootVolumeType" validate="required" required-message="请选择系统盘类别">
        <el-select v-model="addData.configs.rootVolumeType" placeholder="请先选择系统盘类别">
          <el-option v-for="item in categoryData" :label="item.name" :value="item.value" :key="item.value"></el-option>
        </el-select>
        <span class="remark_title">磁盘类型(GB)</span>
      </basic-form-item>
    </el-col>
    <el-col :span="6" :offset="1">
      <basic-form-item label=" " prop="configs.rootSize" label-width="25px" validate="required">
        <el-input-number v-model="addData.configs.rootSize" :min="40" :max="500" style="width: 100%"> </el-input-number>
        <span class="remark_title">磁盘容量(GB)</span>
      </basic-form-item>
    </el-col>
    <el-col :span="24">
    ...
    <basic-form :model="vmData" ref="addForm" label-position="left" :disabled="disabled">
      ...
      <el-col :span="24">
        <el-col :span="12">
          <basic-form-item label="所属网络：" prop="vpcUuid" validate="required" required-message="请选择所属网络">
            <el-select v-model="vmData.vpcUuid" @change="getSubnet">
        ...
        <el-col :span="12">
          <basic-form-item label="所属子网：" prop="subnetUuid" validate="required" required-message="请选择子网">
            <el-select v-model="vmData.subnetUuid" placeholder="请先选择交换机">
      ...
      <el-col :span="24" v-if="vmData.loginType == '密钥'">
        <basic-form-item label="密钥：" prop="keypairName" validate="required" required-message="该字段为必填字段">
          <el-select v-model="vmData.keypairName">
      ...
      <basic-form-item label="安全组：" prop="groups" validate="required" required-message="该字段为必填字段">
        <el-select v-model="vmData.groups" multiple placeholder="请选择">
    ...
    <div slot="preview-card">
      <basic-form-item label="规格：" v-if="vmData.flavorId">{{currentElement.cpu + 'C/' + currentElement.memory + 'GB/' + addData.configs.rootSize + 'GB'}}</basic-form-item>
...
export default {
  components: { CommonWrapper, images, DataDisk, flavor },
  props: {
    type: {
      type: String
    },
  ...
  data () {
    return {
      elements: [{
        name: '云主机',
        serviceCode: 'huawei.standard.server',
        main: true,
        cpu: 0,
        memory: 0
      }, {
      ...
      groupList: [],
      keypairList: [],
      loginWays: [{ name: '密钥' }, { name: '密码' }, { name: '随机密码' }],
      retention: false
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleShowData () {
      ...
      const { name, password, vpcUuid, subnetUuid, groups, loginType, keypairName } = this.addData.configs
      this.vmData = {
        name: name,
        vpcUuid: vpcUuid,
