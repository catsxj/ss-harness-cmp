...
export default defineComponent({
  ...
  setup(props, context) {
    const hostData = reactive({
      // 对数据进行格式化以保持和列表数据一致
      list: props.itemData!.data.map((data: ITargetHost) => {
    ...
    function handleSelectItem(selection: IHost[], row: IHost) {
      if (unref(selectIds).includes(row.id)) {
        const index = unref(selectList).findIndex((item: IHost) => item.id === row.id)
        selectList.value.splice(index, 1)
      } else {
        selectList.value.push(row)
      }
    }
    function handleSelectAll(selection: Array<IHost>) {
      if (selection.length) {
        // 全选情况下
    ...
    function delServer({ id, name }: IHost) {
    ...
    function getPostData() {
      let data: boolean | ITargetHost[] = false
