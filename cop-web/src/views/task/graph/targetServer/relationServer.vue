...
<template>
  <CardLayout title="关联主机">
    <basic-form :disabled="isPreview">
      <el-row :gutter="10">
        <el-col :span="24">
          <el-alert class="m-b-sm" type="warning" :closable="false" title="只有将主机关联到该节点上，主机才会作为该节点的前置依赖，在主机节点上进行脚本执行、软件安装等操作，否则只是单纯的编排">
          </el-alert>
          <smart-table :data="nodeList" class="target-table">
            ...
            <el-table-column label="是否关联">
              <template slot-scope="scope">
                <el-switch
                  @change="changeLink(scope.row)"
                  :disabled="judgeDisLink(scope.row)"
    ...
    changeLink (data) {
      ...
      conditionVm({
        ...
        if (res.success && (!res.data.account || !res.data.password)) {
          this.$message.error('抱歉，无法获取虚拟机认证信息，请自主进行配置')
          data.isLink = false
        }
      })
    },
