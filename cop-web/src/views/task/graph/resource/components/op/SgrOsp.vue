<template>
  <basic-form :model="itemData" ref="form">
    ...
    <basic-form-item label="起始端口：" prop="portMin" v-if="itemData.protocol != 'ICMP'" validate="required"
                   required-message="请输入起始端口">
      <el-input-number v-model="itemData.portMin" :disabled="isPreview" style="width: 100%" :min="1" :max="65535"></el-input-number>
    </basic-form-item>
    <basic-form-item label="结束端口：" prop="portMax" v-if="itemData.protocol != 'ICMP'" validate="required"
                   required-message="请输入结束端口">
