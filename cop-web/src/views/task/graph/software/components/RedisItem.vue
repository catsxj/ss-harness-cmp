    ...
    <el-row>
      ...
      <el-col :span="12" v-if="module === 'cluster'">
        <basic-form-item label="集群模式：" prop="cluster_type" validate="required" :disabled="isTaskInstance">
          <el-select v-model="itemData.cluster_type">
            <el-option v-for="item in redisClusterTypeList" :key="item.value" :label="item.name" :value="item.value">
            </el-option>
          </el-select>
        </basic-form-item>
      </el-col>
...
export default {
  ...
  created () {
    this.getVersion();
    const { manage_config: { redis_password } } = this.itemData;
    if (redis_password) {
    ...
    getPostData () {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          const { manage_config: { redis_password }, cluster_type } = itemData;
          if (redis_password) {
