  ...
  <basic-form :model="itemData" ref="form" :disabled="isPreview" label-wdith="110px">
    ...
    <el-row>
      ...
      <el-col :span="24" class="title">
        新增数据库 <el-button v-if="!isPreview" class="m-l" type="primary" size="mini"  @click="addDbItem">新增</el-button>
      </el-col>
    </el-row>
          ...
          <el-col :span="8">
            <basic-form-item label="名称：" label-width="80px" :prop="`oracle_databases.${index}.init_parameters.${index1}.name`" validate="required">
              <el-select  v-model="item.name" :disabled="isPreview">
                <el-option v-for="row in initNameList" :key="row.value" :label="row.name" :value="row.value">
                </el-option>
              </el-select>
            </basic-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    initData () {
      const { asm_diskgroups, oracle_databases, cluster_install, manage_config, manage_config: { vip } } = this.itemData;
      if (!asm_diskgroups) {
    ...
    getPostData () {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          const { manage_config, oracle_databases, manage_config: { oracle_gi_nic_pub: pub, oracle_gi_nic_priv: priv, vip }, cluster_install } = itemData;
          if (cluster_install) {
