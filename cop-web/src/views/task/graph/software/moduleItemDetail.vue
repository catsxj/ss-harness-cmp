...
<template>
  <el-form :label-width="labelWidth"  :model="item" :disabled="disabled">
    <el-form-item :label="item.name+':'" :title="item.name" prop="defaultValue"
                  :rules="{required:item.required, message: '该属性不能为空' }">
      ...
      <el-table v-else-if="item.type=='TABLE'"
                :data="item.grid"
                empty-text=""
                border
                style="width: 100%">
        <el-table-column v-for="(item1,key) in item.item" :key="key"
                         :label="item1.name">
          ...
          <template slot-scope="scope">
            <el-dropdown @command="tools.handleOperate" placement="bottom-start">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item :disabled="disabled" :command="{flag: 1, index: scope.$index, item: item.grid}">插入行
                </el-dropdown-item>
                <el-dropdown-item :disabled="disabled" :command="{flag: 2, index: scope.$index,  item: item.grid}">删除行
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        ...
        <div slot="empty">
          <el-button type="primary" size="mini" @click="tools.handleOperate({flag: 3, item: item.grid})"><i class="el-icon-plus" />添加数据
          </el-button>
        </div>
      </el-table>
      <el-input v-else-if="item.type=='INTEGER'" v-model.number="item.defaultValue" type="number"
                @blur="validateInteger(item)">
        <template slot="append"  v-if="item.unit">{{item.unit}}</template>
      </el-input>
      <el-input v-model.number="item.defaultValue" type="number" v-else-if="item.type === 'FLOAT'"
                @blur="validateFloat(item)">
        <template slot="append" v-if="item.unit">{{item.unit}}</template>
      </el-input>
      <el-input v-model="item.defaultValue" :type="showType" v-else-if="item.type === 'TEXTENCRYPT'" show-password>
      </el-input>
