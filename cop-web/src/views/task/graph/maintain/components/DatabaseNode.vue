<template>
  <basic-form ref="form" :model="itemData"  :disabled="isPreview" label-width="110px">
    <el-row :gutter="10">
      <el-col :span="12">
        <basic-form-item label="数据库路径：" prop="jdbcUrl" validate="required">
          <el-input v-model="itemData.jdbcUrl"
                    placeholder="192.168.2.216:3306/bsm"></el-input>
        </basic-form-item>
      </el-col>
      <el-col :span="12">
        <basic-form-item label="数据库类型：" prop="database" validate="required">
          <el-select v-model="itemData.database" placeholder="请选择">
            <el-option
              v-for="item in dbListData"
              :key="item.value"
              :label="item.name"
              :value="item.name">
            </el-option>
    ...
    <select-script-item :script-item="itemData"><span v-if="isPreview || isTaskInstance"></span>
    </select-script-item>
  </basic-form>
</template>
<script>
