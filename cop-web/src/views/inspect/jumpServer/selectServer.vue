<template>
  <el-row :gutter="10">
    ...
    <el-col :span="24">
      <el-form-item :label="(isPreview?'CMDB主机:':'')">
        <div class="table-container">
          <el-table :data="itemData.jumpServer.hostList" class="target-table" header-cell-class-name="custom-header" stripe max-height="300">
            ...
            <el-table-column label="端口">
              <template slot-scope="scope">
                <basic-form-item class="m-b-none" :prop="'jumpServer.hostList.'+scope.$index+'.port'" validate="required,port"
                              required-message="请填写端口号">
                  <el-input v-model="scope.row.port" type="number" min="1"></el-input>
                </basic-form-item>
              </template>
            </el-table-column>
            <el-table-column label="用户名" v-if="itemData.jumpServer.authenMode=='SINGLE'">
              <template slot-scope="scope">
                <basic-form-item class="m-b-none" :prop="'jumpServer.hostList.'+scope.$index+'.user'" validate="required"
                              required-message="请填写用户名" key="user">
                  <el-input v-model="scope.row.user"></el-input>
                </basic-form-item>
              </template>
            </el-table-column>
            <el-table-column label="密码" v-if="itemData.jumpServer.authenMode=='SINGLE'">
              <template slot-scope="scope">
                <basic-form-item class="m-b-none" :prop="'jumpServer.hostList.'+scope.$index+'.pasd'"
    ...
    <el-dialog title="CMDB主机" :close-on-click-modal="false" :visible.sync="dialogServerCmdbVisible" append-to-body
      ...
      <!-- 选择cmdb服务器 -->
      <select-host ref="cmdbSelectHost" :selectData="itemData.jumpServer.hostList"></select-host>
