<template>
  <el-row :gutter="10">
    ...
    <el-dialog title="安全设备列表" :close-on-click-modal="false" :visible.sync="dialogServerVisible" append-to-body
               width="60%" v-if="dialogServerVisible">
      <select-network
        ref="cmdbSelectNetwork"
        :selectData="selectData"
        dictType="CMDB_SECURITY_DEVICE"
...
export default {
  ...
  methods: {
    ...
    clearServer () {
      this.$confirm('您确定要清空所选安全设备吗?', '操作确认', {
    ...
    delServer (id) {
      this.$confirm('您确定要删除该安全设备吗?', '操作确认', {
        confirmButtonClass: 'el-button--danger',
        type: 'warning'
      }).then(() => {
        this.clearCustomParams()
