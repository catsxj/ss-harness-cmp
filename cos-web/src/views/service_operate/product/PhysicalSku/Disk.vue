  ...
  <div>
    <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="columns" :get-list="getData" :total="total" :loading="loading">
      <template v-slot:action>
        ...
        <ImportData url="/api/ims/v1/spec/disk/import" @getData="getData" template-url="/ims/v1/spec/disk/import/template"></ImportData>
        <el-button icon="el-icon-download" @click="exportDiskList"> 导出 </el-button>
      </template>
      <template #status="val">
    ...
    <DiskAdd v-if="addData.visible" :addData="addData" @success="getData" />
  </div>
</template>

<script>
import { getDiskSpecs, exportDiskList, deleteDisk, modifyDiskStatus, getBrand } from 'services/services/physicalSpec.js'
...
export default {
  ...
  data() {
    return {
      searchConfigs: [
        ...
        { type: 'Input', label: '硬盘类型', value: 'diskType' },
  ...
  methods: {
    exportDiskList() {
      exportDiskList(this.params)
    },
    async handleStatus(record) {
      const text = record.status ? '禁用' : '启用'
      const confirm = await this.$confirm(`确定要${text}该硬盘规格吗？`, '提示', {
        type: 'warning'
      })
      if (!confirm) return
      const data = await modifyDiskStatus(record.id, !record.status)
    ...
    async handleDelete(record) {
      const confirm = await this.$confirm('确定要删除该硬盘规格吗？', '提示', {
        type: 'warning'
      })
      if (!confirm) return
      const data = await deleteDisk(record.id)
      if (!data.success) return
      this.$message.success(data.message)
      this.getData()
    },
    async getData() {
      this.loading = true
      const data = await getDiskSpecs(this.params).finally(() => (this.loading = false))
      if (!data.success) return
