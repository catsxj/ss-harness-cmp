          ...
          <el-row :gutter="20">
            <el-col v-for="(row, key) in serviceList" :key="row.id" class="m-b card-container">
              <el-card class="box-card service-card">
                ...
                <div class="footer">
                  <div class="pull-left footer-category">{{ row.flowName }}</div>
                  <el-button class="operate pull-right" type="ghost" @click="openDialog(row)">绑定</el-button>
                  <el-button v-if="row.flowId" class="operate pull-right m-r" type="ghost" @click="unBind(row)">解绑 </el-button>
                </div>
              </el-card>
            </el-col>
          </el-row>
...
import { flowBindConfig, modifyFlowBindConfig } from 'services/services/catelog'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    bindFlow() {
      ...
      modifyFlowBindConfig(this.bindData.id, formData).then((data) => {
        if (data.success) {
