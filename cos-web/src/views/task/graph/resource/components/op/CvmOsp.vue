<template>
  <basic-form :model="itemData" ref="formRef" :disabled="isPreview">
    ...
    <div v-if="!isTemplate">
      <vm-server class="task-node" :no-task="false" v-if="itemData.origin === 'create'" ref="nodeRef" :item-data="applyData" type="OPENSTACK" :template="isTemplate" :disabled="isPreview"></vm-server>
      <div v-else>
        <RegionItem :addData="itemData" @changeVendorId="getData" vendorType="OPENSTACK"></RegionItem>
        <h5>配置信息</h5>
        <basic-form-item label="云主机列表：" prop="id" validate="required" :disabledValidate="isTemplate">
...
import vmServer from '@/views/resource-apply/ecs/op/index.vue'
