  ...
  <el-row :gutter="10">
    ...
    <el-col :span="24">
      <el-form-item label="文件列表:">
        <smart-table :data="fileList">
          <el-table-column align="left" label="文件列表" prop="name" show-overflow-tooltip> </el-table-column>
          ...
          <el-table-column align="left" label="操作" width="220" v-if="!isPreview && !isTaskInstance">
            <template slot-scope="scope">
              <el-button v-show="scope.row.file" size="mini" type="primary" @click="checkSoft(scope.row)" :disabled="scope.row.isReady || scope.row.isSuccess || scope.row.isUploading"> 上传 </el-button>
              <el-button v-show="scope.row.file" size="mini" type="danger" @click="delServer(scope.row.host)" :disabled="!scope.row.isUploading"> 取消 </el-button>
              <el-button size="mini" type="danger" @click="delFile(scope.$index)" :disabled="isPreview"> 移除 </el-button>
            </template>
          </el-table-column>
        </smart-table>
      </el-form-item>
    ...
    <el-dialog title="文件列表" :close-on-click-modal="false" :visible.sync="dialogFileVisible" append-to-body v-if="dialogFileVisible">
      ...
      <basic-table :data="list" ref="fileTable" :params="params" :get-list="getList" :total="total" @select="handleSelectItem" @select-all="handleSelectAll">
        ...
        <el-table-column label="文件分组" prop="groupName"> </el-table-column>
