<template>
  <el-select filterable :value="sceneId" placeholder="请选择作业场景" @change="changeData" clearable :disabled="disabled">
  ...
  setup(props, context) {
    ...
    function getData() {
      getScene({
        ...
        if (data.success) {
          const arr = data.data.rows.filter(({ name }: any) => name === '软件安装')
