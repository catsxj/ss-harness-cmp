<template>
  <common-wrapper code="storage" @backZone="zoneChange" :add-data="addData" ref="common" :elements="elements" :get-params="getParams" :item-data="retention" :disabled="disabled" v-if="isLoadData">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="10" v-if="addData.configs.hasServer" key="server">
            <basic-form-item label="云主机：" prop="serverId" validate="required">
              <el-select v-model="addData.configs.serverId" placeholder="请选择云主机">
          ...
          <el-col :span="10">
            <basic-form-item label="磁盘类别：" prop="diskType" validate="required">
              <el-select v-model="addData.configs.diskType" placeholder="请先选择磁盘类别">
                <el-option v-for="item in volumeTypeData" :label="item.name" :value="item.volumeTypeUuid" :key="item.volumeTypeUuid"></el-option>
              </el-select>
            </basic-form-item>
          </el-col>
          <el-col :span="10">
...
import { getVm, getDiskType } from 'services/platform/index'
...
export default {
  ...
  data() {
    return {
      elements: [
        {
          ...element,
          name: '云硬盘',
          serviceCode: 'cecstack.standard.volume',
          insAmount: 20,
          main: true
        }
      ],
      addData: {
        ...
        service: 'cecstack.standard.volume',
        configs: {
          resourceLabel: [],
          hasServer: false,
          diskType: ''
  ...
  methods: {
    ...
    getDiskType() {
      const params = {
        simple: true,
        params: JSON.stringify([{ param: { vendorId: this.addData.location.vendorId, regionId: this.addData.location.region, zone: this.addData.location.az }, sign: 'EQ' }])
      }
      getDiskType(params).then((data) => {
        if (data.success) {
          this.volumeTypeData = data.data.rows
          if (this.addData.configs.diskType) {
            const find = this.volumeTypeData.find(({ volumeTypeUuid }) => volumeTypeUuid === this.addData.configs.diskType)
            if (!find) this.addData.configs.diskType = this.volumeTypeData?.[0].volumeTypeUuid || ''
          }
        }
      })
    },
