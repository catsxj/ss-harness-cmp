              ...
              <el-row class="rule-box">
                <el-col :span="24" v-for="(row, index) in addData.rules" :key="index" class="m-t">
                  <el-row :gutter="10">
                    ...
                    <el-col :span="5">
                      <el-input-number style="width: 100%" v-model="row.value" controls-position="right" :min="0" :max="9999999999999999"></el-input-number>
                    </el-col>
                    <el-col :span="4">
          ...
          <div v-if="analysisStatus == 'history'">
            ...
            <el-col :span="24">
              <SmartTable :data="tableList">
                ...
                <el-table-column show-overflow-tooltip label="磁盘使用情况(KBps)" prop="diskUsage"> </el-table-column>

                <el-table-column show-overflow-tooltip label="网络使用情况(KBps)" prop="netUsage"> </el-table-column>
                <el-table-column show-overflow-tooltip label="所属用户" prop="ownerName"> </el-table-column>
                <el-table-column show-overflow-tooltip label="所属租户" prop="tenantName"> </el-table-column>
...
const searchConfigs = [
  ...
  { type: 'Const', value: 'type', initValue: 'wholeMachineRecycle' }
]
  ...
  methods: {
    ...
    handleCreate(data) {
      if (data) {
        ...
        if (this.addData.rules.length == 1) {
          if (this.addData.rules[0].metric == 'diskUsageAverage') {
            this.addData.rules.push({ metric: 'netUsageAverage', operator: '', value: '', valueType: '', relation: '' })
          } else {
            this.addData.rules.unshift({ metric: 'diskUsageAverage', operator: '', value: '', valueType: '', relation: '' })
          }
        }
        this.addFlag = true
      } else {
        this.addData = {
          name: '',
          type: 'wholeMachineRecycle',
          rules: [
            { metric: 'diskUsageAverage', operator: '', value: '', valueType: '', relation: '' },
            { metric: 'netUsageAverage', operator: '', value: '', valueType: '', relation: '' }
          ]
        }
      ...
      this.$refs.analysisData.validate((valid) => {
        if (valid) {
          ...
          if (this.analysisData.vendorId.includes(-1)) {
            // 全部
            params.types = ['OPENSTACK', 'VMWARE', 'INSPURRAIL', 'MANAGEONE', 'FUSIONSPHERE', 'USPHERE', 'SANGFOR']
          } else {
    ...
    getVendorData() {
      conditionCloudVendor({
        condition: JSON.stringify({
          ...
          types: ['OPENSTACK', 'VMWARE', 'INSPURRAIL', 'MANAGEONE', 'FUSIONSPHERE', 'USPHERE', 'SANGFOR']
        })
      }).then((data) => {
        if (data.success) {
