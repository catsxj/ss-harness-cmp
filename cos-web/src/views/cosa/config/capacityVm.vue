  ...
  data() {
    return {
      searchConfigs: [
        ...
        { type: 'Const', value: 'type', initValue: 'cloudServerCapacityExpansion' }
      ],
      relationList,
      relationType,
      valueType,
  ...
  methods: {
    ...
    handleCreate(data) {
      if (data) {
        ...
        this.addData = {
          name: '',
          type: 'cloudServerCapacityExpansion',
          rules: [
      ...
      this.$refs.analysisData.validate((valid) => {
        if (valid) {
          ...
          if (this.analysisData.vendorId.includes(-1)) {
            // 全部
            params.types = ['OPENSTACK', 'VMWARE', 'INSPURRAIL', 'CECSTACK', 'CNWARE', 'EASYSTACK', 'ARCHEROS', 'CLOUDTOWER', 'FUSIONSPHERE', 'USPHERE', 'SANGFOR']
          } else {
    ...
    getVendorData() {
      conditionCloudVendor({
        condition: JSON.stringify({
          ...
          types: ['OPENSTACK', 'VMWARE', 'INSPURRAIL', 'CECSTACK', 'CNWARE', 'EASYSTACK', 'ARCHEROS', 'CLOUDTOWER', 'FUSIONSPHERE', 'USPHERE', 'SANGFOR']
        })
      }).then((data) => {
        if (data.success) {
