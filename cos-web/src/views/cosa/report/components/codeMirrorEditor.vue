...
import { filterMode } from '../utils/index'
...
export default {
  ...
  data() {
    return {
      editorValue: this.codes,
      cmOptions: {
        theme: 'xq-light',
        mode: filterMode(this.cmMode),
        lineWrapping: true,
        lineNumbers: true,
        autofocus: true,
        smartIndent: false,
  ...
  watch: {
    cmMode: function(newValue, oldValue) {
      this.$set(this.cmOptions, 'mode', filterMode(this.cmMode))
      this.resetLint()
  ...
  methods: {
    ...
    // 插入字段
    insertData(data) {
      const pos = this.$refs.myCm.codemirror.getCursor()
      const { line, ch } = pos
      this.$refs.myCm.codemirror.replaceRange(data, { line, ch })
    }
  },
  created() {
    try {
