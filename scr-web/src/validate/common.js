...
export default {
  ...
  vmPassword: {
    validator: (rule, value, callback) => {
      const pattern = '^(?![a-zA-Z]+$)(?![A-Z0-9]+$)(?![A-Z\\W_]+$)(?![a-z0-9]+$)(?![a-z\\W_]+$)(?![0-9\\W_]+$)[a-zA-Z0-9\\W_]{12,36}$'
      if (!value.match(pattern)) {
        callback(new Error('同时包含大小写字母，数字或特殊字符任意三项,长度介于12-36字符'))
      } else {
        callback()
      }
    },
    ...
    validator: (rule, value, callback) => {
      ...
      function getValLen (value, strlen) {
        for (let i = 0; i < value.length; i++) {
          if (value.charCodeAt(i) < 27 || value.charCodeAt(i) > 126) { // 中文和中文字符
            strlen += 3;
          } else {
            strlen++;
          }
