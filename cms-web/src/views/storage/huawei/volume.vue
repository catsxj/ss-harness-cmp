<template>
  <div class="wrapper">
    ...
    <common-detail v-if="detailFlag" :setting="{ logoText: '存储卷' }" :title="detailData.name" @goBack="goBack">
...
import { getVolumes, getStorages } from 'services/monitor/storage'
import overview from './volume_detail.vue'
...
export default {
  ...
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: this.projectName == 'HUAWEI' ? 'MONITOR_STORAGE_HUAWEI_VOLUME' : 'MONITOR_STORAGE_DELL_VOLUME'
    }
  },
  methods: {
    getList() {
      this.loading = true
      this.refreshId()
      getVolumes(this.params).then((data) => {
        this.loading = false
        if (data.success) {
    ...
    getDetail(row) {
      ...
      this.detailUuid = row.volumeUuid
