...
<script>
import { statusFilter, stautsIconFilter, quotaFilter, capacityFilter } from './filters'
import { getRuleGroupBind } from 'services/monitor/index.js'
import { getObjectUser } from 'services/monitor/xsky.js'
import detail from './objectUserDetail.vue'
...
export default {
  ...
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_XSKY_OBJECT_USER'
    }
  },
  computed: {
    paramsStr() {
      const { vendorId, userId } = this.detailData
      return `vendorId = "${vendorId}", id = "${userId}"`
    }
  },
  methods: {
    getList() {
