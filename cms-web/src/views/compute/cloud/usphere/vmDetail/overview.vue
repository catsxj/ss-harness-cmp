...
export default {
      ...
      typeData: [
        {
          title: 'CPU利用率',
          value: 'cpu',
          expr: JSON.stringify(['round(bocloud_uis_vm_cpu{instanceId="' + this.detailUuid + '"},0.01)']),
          name: '["CPU利用率"]',
          unit: '%'
        },
        {
        ...
        },
        {
          title: '内存利用率',
          value: 'mem',
          expr: JSON.stringify(['round(bocloud_uis_vm_mem{instanceId="' + this.detailUuid + '"},0.01)']),
          name: '["内存利用率"]',
          unit: '%'
        }
      ],
  ...
  methods: {
    ...
    getDashboard() {
      const expr1 = 'round(bocloud_uis_vm_cpu{instanceId="' + this.detailUuid + '"},0.01)'
      const expr2 = 'round(bocloud_uis_vm_mem{instanceId="' + this.detailUuid + '"},0.01)'
      getHostDashboard({
        expr: expr1,
        simple: true
      }).then((data) => {
