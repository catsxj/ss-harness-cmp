<template>
  <div>
    <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="vmColumns" :get-list="getList" :total="total" :loading="loading" @select="handleSelectItem" @select-all="handleSelectAll" ref="resourceMonitorTable">
      ...
      <template #diskUsage="diskUsage">
        <el-progress class="progress" :color="customColorMethod" text-inside :stroke-width="16" :percentage="diskUsage ? diskUsage : 0"></el-progress>
      </template>
    ...
    <add-policy v-if="addFlag" resourceType="MONITOR_USPHERE_VM" @cancle="cancle" :add-link-flag="addFlag" :resourceId="resourceId" :ruleGroupIds="ruleGroupIds"></add-policy>
...
export default {
    ...
    return {
      ...
      vmColumns: [
        ...
        },
        {
          label: '磁盘利用率',
          prop: 'diskUsage',
          scopedSlots: { customRender: 'diskUsage' }
        },
        // {
        //   label: '通断状态',
        //   prop: 'onOffStatus',
      ...
      searchConfigs: [
        ...
        { type: 'Const', value: 'vendorType', initValue: 'USPHERE' },
  ...
  methods: {
    ...
    handlePolicy(data) {
      this.handleStart(this, data, 'MONITOR_USPHERE_VM')
    },
    getDetail(row) {
    ...
    handleMonitorDetail(id, name) {
      this.$router.push({
        ...
        query: { id: id, name: name, type: 'MONITOR_USPHERE_VM' }
      })
    },
    setIP(row) {
      this.setData = {
