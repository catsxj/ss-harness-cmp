...
export default {
      ...
      typeData: [
        {
          title: 'CPU利用率',
          value: 'cpu',
          expr: JSON.stringify(['round(bocloud_uis_host_cpu{instanceId="' + this.detailUuid + '"}, 0.01)']),
          name: '["cpu利用率"]',
          unit: '%'
        },
        // {
        //   title: '磁盘使用速率',
        //   value: 'disk',
        //   expr: JSON.stringify(['round(bocloud_uis_host_STORAGE{instanceId="' + this.detailUuid + '"},0.01)']),
        //   name: "['磁盘使用速率']",
        //   unit: 'KB/s'
        // },
        {
          title: '内存利用率',
          value: 'mem',
          expr: JSON.stringify(['round(bocloud_uis_host_mem{instanceId="' + this.detailUuid + '"}, 0.01)']),
          name: "['内存利用率']",
          unit: '%'
        }
      ],
  ...
  methods: {
    getDetail() {
      getHostDetail(this.detailId, 'USPHERE').then((data) => {
        if (data.success) {
    ...
    getDashboard() {
      const expr1 = 'round(bocloud_uis_host_cpu{instanceId="' + this.detailUuid + '"},0.01)'
      const expr2 = 'round(bocloud_uis_host_mem{instanceId="' + this.detailUuid + '"},0.01)'
      getHostDashboard({
        expr: expr1,
        simple: true
      }).then((data) => {
