<template>
  <div>
    ...
    <add-policy v-if="addFlag" resourceType="MONITOR_CNWARE_VM" @cancle="cancle" :add-link-flag="addFlag" :resourceId="resourceId" :ruleGroupIds="ruleGroupIds"></add-policy>
    ...
    <common-detail v-if="detailFlag" :title="detailData.name" @goBack="goBack">
      <div slot="custom_content">
        <detail :detail-id="detailId" :detailUuid="detailUuid" :detailVendorId="detailVendorId"></detail>
      </div>
    </common-detail>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      searchConfigs: [
        ...
        { type: 'Const', value: 'vendorType', initValue: 'CNWARE' },
  ...
  methods: {
    ...
    handlePolicy(data) {
      this.handleStart(this, data, 'MONITOR_CNWARE_VM')
    },
    getDetail(row) {
    ...
    handleMonitorDetail(id, name) {
      this.$router.push({
        ...
        query: { id: id, name: name, type: 'MONITOR_CNWARE_VM' }
      })
    },
    setIP(row) {
      this.setData = {
