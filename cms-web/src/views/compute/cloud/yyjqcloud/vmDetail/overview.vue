...
export default {
  ...
  data() {
    return {
      typeData: [
        ...
        //   title: '磁盘利用率',
        //   value: 'disk',
        //   expr: JSON.stringify(['round(yyjqcloud_cvm_disk{instance_id="' + this.instanceId + '"}, 0.01)']),
        //   name: "['磁盘利用率']",
        //   unit: '%'
        // }
      ],
  ...
  methods: {
    ...
    getDashboard() {
      const expr1 = 'round(yyjqcloud_cvm_cpu{instance_id="' + this.instanceId + '"}, 0.01)'
      const expr2 = 'round(yyjqcloud_cvm_memory{instance_id="' + this.instanceId + '"}, 0.01)'
      const expr3 = 'round(yyjqcloud_cvm_disk{instance_id="' + this.instanceId + '"}, 0.01)'
      getHostDashboard({
        expr: expr1,
        simple: true
      }).then((data) => {
