...
export default {
  ...
  data() {
    return {
      typeData: [
        {
          title: 'CPU利用率',
          value: 'cpu',
          expr: JSON.stringify(['bocloud_lenovo_vm_cpu_usage{instanceId="' + this.instanceId + '"}']),
          name: '["CPU利用率"]',
          unit: '%'
        },
        {
      ...
      detailData: {},
      detailMapList: [
        { name: 'IP', value: 'privateIps', unit: '', type: 'lenovo' },
        { name: '规格', value: 'spec' },
  ...
  methods: {
    ...
    getDashboard() {
      const expr1 = 'bocloud_lenovo_vm_cpu_usage{instanceId="' + this.instanceId + '"}'
      const expr2 = 'bocloud_lenovo_vm_memory_usage{instanceId="' + this.instanceId + '"}'
      getHostDashboard({
        expr: expr1,
        simple: true
      }).then((data) => {
