...
export default {
  ...
  data() {
    return {
      typeData: [
        {
          title: 'CPU利用率',
          value: 'cpu',
          expr: JSON.stringify(['round(bocloud_inspurRail_vm_cpu{id="' + this.detailId + '"},0.01)']),
          name: '["CPU利用率"]',
          unit: '%'
        },
        {
          title: '磁盘使用速率',
          value: 'disk',
          expr: JSON.stringify(['round(bocloud_inspurRail_vm_storage{id="' + this.detailId + '"},0.01)']),
          name: '["磁盘使用速率"]',
          unit: 'KB/s'
        },
        {
          title: '内存利用率',
          value: 'mem',
          expr: JSON.stringify(['round(bocloud_inspurRail_vm_mem{id="' + this.detailId + '"},0.01)']),
          name: '["内存利用率"]',
          unit: '%'
        }
      ],
      ...
      detailMapList: [
        ...
        { name: 'IP', value: 'privateIps', unit: '', type: 'inspurRail' },
        { name: '规格', value: 'spec' },
  ...
  methods: {
    ...
    getDashboard() {
      const expr1 = 'round(bocloud_inspurRail_vm_cpu{id="' + this.detailId + '"},0.01)'
      const expr2 = 'round(bocloud_inspurRail_vm_mem{id="' + this.detailId + '"},0.01)'
      getHostDashboard({
        expr: expr1,
        simple: true
      }).then((data) => {
