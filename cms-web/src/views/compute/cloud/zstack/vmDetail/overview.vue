...
export default {
  ...
  data() {
    return {
      typeData: [
        {
          title: 'CPU利用率',
          value: 'cpu',
          expr: JSON.stringify(['round(bocloud_zstack_vm_cpu_util{instance_id="' + this.instanceId + '"},0.01)']),
          name: '["CPU利用率"]',
          unit: '%'
        },
        {
          title: '磁盘使用速率',
          value: 'disk',
          expr: JSON.stringify(['round(bocloud_zstack_vm_disk_util_rate{instance_id="' + this.instanceId + '"},0.01)']),
          name: '["磁盘使用速率"]',
          unit: 'KB/s'
        },
        {
          title: '内存利用率',
          value: 'mem',
          expr: JSON.stringify(['round(bocloud_zstack_vm_mem_util{instance_id="' + this.instanceId + '"},0.01)']),
          name: '["内存利用率"]',
          unit: '%'
        }
      ],
      ...
      detailMapList: [
        ...
        { name: 'IP', value: 'privateIps', unit: '', type: 'zstack' },
        { name: '规格', value: 'spec' },
  ...
  methods: {
    getDetail() {
      detailVm(this.detailId).then((data) => {
        if (data.success) {
          ...
          this.detailData.privateIps = JSON.parse(this.detailData.privateIps)[0].address
    ...
    getDashboard() {
      const expr1 = 'round(bocloud_zstack_vm_cpu_util{instance_id="' + this.instanceId + '"},0.01)'
      const expr2 = 'round(bocloud_zstack_vm_mem_util{instance_id="' + this.instanceId + '"},0.01)'
      getHostDashboard({
        expr: expr1,
        simple: true
      }).then((data) => {
