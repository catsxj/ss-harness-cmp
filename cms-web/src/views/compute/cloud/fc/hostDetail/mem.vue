...
export default {
  ...
  data () {
    return {
      typeData: [
        // { title: '内存利用率', value: 'usage', expr: JSON.stringify(['round(vsphere_host_mem_usage_average{esxhostname="' + this.detailName + '"}, 0.01)']), name: "['内存利用率']", unit: '%' }
        { title: '内存利用率', value: 'usage', expr: JSON.stringify(['round(bocloud_fusionsphere_host_mem_usage{vendorId="' + this.detail.vendorId + '",instance_id="' + this.detail.uuid + '"}, 0.01)']), name: "['内存利用率']", unit: '%' }
      ],
      detailData: {},
      detailMapList: [
