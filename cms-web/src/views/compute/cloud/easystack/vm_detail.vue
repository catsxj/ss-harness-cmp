...
export default {
      ...
      typeData: [
        {
          title: 'CPU利用率',
          value: 'first',
          expr: JSON.stringify(['round(easystack_vm_cpu_util{id="' + this.instanceId + '"} ,0.01)']),
          name: '["CPU利用率"]',
          unit: '%'
        },
        {
          title: '内存利用率',
          value: 'second',
          expr: JSON.stringify(['round(easystack_vm_memory_util{id="' + this.instanceId + '"},0.01)']),
          name: '["内存利用率"]',
          unit: '%'
        },
        // {
        ...
        // },
        {
          title: '磁盘读写速度',
          value: 'fourth',
          expr: JSON.stringify(['easystack_vm_disk_read_bytes_rate{id="' + this.instanceId + '"}', 'easystack_vm_disk_write_bytes_rate{id="' + this.instanceId + '"}']),
          name: '["读取","写入"]',
          unit: 'B/s'
        }
        // {
        ...
        //   title: '网络收发数据包',
        //   value: 'sixth',
        //   expr: JSON.stringify(['easystack_vm_network_incoming_packets_rate{id="' + this.instanceId + '"}', 'easystack_vm_network_outgoing_packets_rate{id="' + this.instanceId + '"}']),
        //   name: '["传入","传出"]',
        //   unit: 'B/s'
        // }
        // {
  ...
  methods: {
    ...
    getCpuUsage() {
      const expr = 'round(easystack_vm_cpu_util{id="' + this.instanceId + '"} ,0.01)'
      getUsage({
        expr: expr,
        simple: true
      }).then((data) => {
    ...
    getMemUsage() {
      const expr = 'round(easystack_vm_memory_util{id="' + this.instanceId + '"},0.01)'
      getUsage({
        expr: expr,
        simple: true
      }).then((data) => {
