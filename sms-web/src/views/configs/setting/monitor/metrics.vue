<template>
  <div>
    <common-detail v-if="visible1" :setting="{ type: 'host' }" :title="titleName" @goBack="goBack">
      <template v-slot:item_container>
        <AdvanceTable :data="tableData" :search-configs="searchConfigs" :params="params" :columns="columns" :get-list="getData" :total="total" ref="tableRef" @selection-change="handleSelectionChange">
          <template #action>
            <el-button type="primary" @click="open()">新增</el-button>
            <el-button type="primary" @click="confirmBind">关联</el-button>
          </template>
          <template #isEvent="val">
            {{ !!val ? '是' : '否' }}
          </template>
          <template #operate="val, record">
            <el-button type="text" @click="open(record)">编辑</el-button>
            <el-button type="text" @click="delMetric(record.id)">删除</el-button>
          </template>
        </AdvanceTable>
      </template>
    </common-detail>
    <el-dialog width="960px" :visible.sync="dialogVisible" :close-on-click-modal="false" title="指标">
      <basic-form :model="dialogData" label-width="150px" ref="formRef">
        <el-row>
          <el-col :span="12">
            <basic-form-item label="指标名称" prop="name" validate="required">
              <el-input type="text" v-model="dialogData.name"></el-input>
            </basic-form-item>
          </el-col>
          <el-col :span="12">
            <basic-form-item label="资源类型" prop="resourceType">
              <el-input type="text" v-model="resourceType" disabled></el-input>
            </basic-form-item>
          </el-col>
          <el-col :span="24">
            <basic-form-item label="表达式" prop="expr" validate="required">
              <el-input type="text" v-model="dialogData.expr"></el-input>
            </basic-form-item>
          </el-col>
          <el-col :span="12">
            <basic-form-item label="是否是事件告警" prop="isEvent" validate="required">
              <el-switch v-model="dialogData.isEvent"></el-switch>
            </basic-form-item>
          </el-col>
          <el-col :span="12">
            <basic-form-item label="告警名称" prop="alarmName">
              <el-input type="text" v-model="dialogData.alarmName"></el-input>
            </basic-form-item>
          </el-col>
          <el-col :span="12">
            <basic-form-item label="指标分组标签" prop="tag" validate="required">
              <el-input type="text" v-model="dialogData.tag"></el-input>
            </basic-form-item>
          </el-col>
          <el-col :span="12">
            <basic-form-item label="单位" prop="unit">
              <el-input type="text" v-model="dialogData.unit"></el-input>
            </basic-form-item>
          </el-col>
          <el-col :span="12">
            <basic-form-item label="规则范围" prop="fun" validate="required">
              <el-checkbox-group v-model="dialogData.fun">
                <el-checkbox v-for="func in funcs" :label="func.value" :key="func.value">{{ func.label }}</el-checkbox>
              </el-checkbox-group>
            </basic-form-item>
          </el-col>
        </el-row>
      </basic-form>
      <div slot="footer">
        <el-button @click.native="dialogVisible = false">取消</el-button>
        <el-button type="primary" @click.native="confirmCreate">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
<script>
import { defineComponent, reactive, ref, toRefs, nextTick, onMounted } from '@vue/composition-api'
import { getMonitorList, getMonitorIds, bindMonitorList, createMetric, updateMetric, deleteMetric } from 'services/services/monitor'
import { MessageBox, Message } from 'element-ui'
import { debounce } from 'lodash-es'

const columns = [
  { type: 'selection' },
  {
    label: '名称',
    prop: 'name'
  },
  {
    label: '表达式',
    prop: 'expr'
  },
  {
    label: '是否是事件告警',
    prop: 'isEvent',
    scopedSlots: { customRender: 'isEvent' }
  },
  {
    label: '告警名称',
    prop: 'alarmName'
  },
  {
    label: '标签',
    prop: 'tag'
  },
  {
    label: '单位',
    prop: 'unit'
  },
  {
    label: '操作',
    width: '150px',
    scopedSlots: { customRender: 'operate' }
  }
]

const funcs = [
  { label: '最小值', value: 'min' },
  { label: '最大值', value: 'max' },
  { label: '平均值', value: 'avg' }
]

export default defineComponent({
  props: {
    visible: {
      type: Boolean,
      default: false
    },
    resourceType: {
      type: String
    },
    titleName: {
      type: String
    }
  },
  setup(props, { emit }) {
    const visible1 = ref(props.visible)
    const tableRef = ref(null)
    const state = reactive({
      tableData: [],
      params: {
        page: 1,
        rows: 10
      },
      total: 0,
      dialogData: {},
      dialogVisible: false
    })
    const selectIds = ref([])
    onMounted(async () => {
      getSelectedIds()
    })
    const getSelectedIds = async () => {
      const { data, success } = await getMonitorIds({ resourceType: props.resourceType })
      if (success) {
        selectIds.value = data
      }
    }
    const getData = async () => {
      const { data, success } = await getMonitorList(state.params)
      if (success) {
        state.tableData = data.rows
        state.total = data.total
        state.tableData.forEach(async (rows) => {
          if (selectIds.value.includes(rows.id)) {
            await nextTick()
            tableRef.value.toggleRowSelection(rows)
          }
        })
      }
    }
    const handleSelectionChange = debounce((selection) => {
      const selectionIds = selection.map((item) => item.id)
      state.tableData
        .map((item) => item.id)
        .forEach((item) => {
          // 列表id在已关联ids中的位置
          const idx = selectIds.value.findIndex((el) => el == item)
          if (selectionIds.includes(item)) {
            // 如果selectionIds含有该列表id，证明被选择，当已关联ids没有该元素，将其添加
            if (idx < 0) selectIds.value.push(item)
          } else {
            // 如果selectionIds没有该列表id，证明取消选择，当已关联ids有该元素，将其删除
            if (idx >= 0) selectIds.value.splice(idx, 1)
          }
        })
    }, 300)
    const goBack = () => {
      emit('update:visible', false)
    }
    const open = (record) => {
      state.dialogData = record ?? { fun: [] }
      state.dialogVisible = true
    }
    const formRef = ref(null)
    const confirmCreate = () => {
      formRef.value.validate(async (valid) => {
        if (valid) {
          const fn = state.dialogData.id ? updateMetric : createMetric
          const { success, message } = await fn({ ...state.dialogData, strFun: JSON.stringify(state.dialogData.fun), resourceType: props.resourceType }, state.dialogData.id)
          if (success) {
            Message.success(message)
            state.dialogVisible = false
            getData()
          }
        }
      })
    }
    const delMetric = async (id) => {
      MessageBox.confirm('您确定要删除该指标吗？', '提示', {
        confirmButtonClass: 'el-button--danger',
        type: 'warning'
      }).then(() => {
        deleteMetric(id).then(({ success, message }) => {
          if (success) {
            Message.success(message)
            getData()
          }
        })
      })
    }
    const confirmBind = async () => {
      const params = {
        metricIds: selectIds.value,
        resourceType: props.resourceType
      }
      const { success, message } = await bindMonitorList(params)
      if (success) {
        Message.success(message)
      }
    }
    return {
      visible1,
      columns,
      searchConfigs: [{ type: 'Const', value: 'resourceType', initValue: props.resourceType }],
      ...toRefs(state),
      tableRef,
      funcs,
      formRef,
      getData,
      handleSelectionChange,
      goBack,
      open,
      confirmCreate,
      delMetric,
      confirmBind
    }
  }
})
</script>
