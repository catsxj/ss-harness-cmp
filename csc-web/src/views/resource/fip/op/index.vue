  ...
  <div class="wrapper">
    <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="columns" :get-list="getData" :total="total" :loading="loading" ref="multipleTable">
      <template v-slot:action>
        <router-link :to="{ name: 'ServiceFloatIpCreate', params: { type: 'OPENSTACK' } }" class="m-l-xs">
      ...
      <template #tag="tag, record">
        <tag :detail="record" type="FLOATING_IP"></tag>
      </template>
...
const searchConfigs = [
  ...
  { type: 'Const', value: 'vendorType', initValue: 'OPENSTACK' }
]

export default {
  components: { selectionService, UnsubscribeService, tag },
  filters: {
