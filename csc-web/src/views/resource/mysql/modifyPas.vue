<template>
  <div>
    <el-dialog title="修改密码" :visible.sync="addData.dialog" width="40%">
      <span>
        <el-form ref="passwordData" :model="addData.data" :status-icon="true" label-width="100px">
          ...
          <el-form-item label="确认密码：" prop="endPassword" :rules="rules1">
            <el-popover ref="popover1" placement="right" trigger="hover">
              <div>
                <span v-if="addData.data.endPassword">
            ...
            <el-input type="password" v-model="addData.data.endPassword" v-popover:popover1 show-password></el-input>
          </el-form-item>
        </el-form>
      </span>
...
export default {
  ...
  watch: {
    ...
    'addData.data.endPassword'() {
      const reg = new RegExp('^(?![a-zA-Z]+$)(?![A-Z0-9]+$)(?![A-Z_!@#$%^&*`~()-+=]+$)(?![a-z0-9]+$)(?![a-z_!@#$%^&*`~()-+=]+$)(?![0-9_!@#$%^&*`~()-+=]+$)[a-zA-Z0-9_!@#$%^&*`~()-+=]{8,30}$')
      this.popoverObj = {
        length: this.addData.data.endPassword.length >= 8 && this.addData.data.endPassword.length <= 128,
        intension: reg.test(this.addData.data.endPassword),
        indexOf: this.addData.data.endPassword == this.addData.data.password
      }
    }
  },
  data() {
