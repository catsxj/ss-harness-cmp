  ...
  <div class="wrapper">
    <common-detail v-if="detailFlag" :title="installDetail.name" @goBack="goBack">
      <template v-slot:item_container>
        ...
        <common-detail-item label="网段">{{ installDetail.cidr }}</common-detail-item>
      ...
      <el-tabs v-model="activeTab" @tab-click="changeTab">
        <el-tab-pane label="子网列表" name="second">
          ...
          <basic-table v-if="activeTab === 'second'" :data="childrenData" :params="paramd" :get-list="getSubnet" :total="childrenDataTotal" ref="childrenMultipleTable">
            ...
            <el-table-column label="已用IP/可用IP" show-overflow-tooltip>
              <template slot-scope="scope"> {{ scope.row.usingIps }}/{{ scope.row.availableIps }} </template>
            </el-table-column>
    ...
    <AdvanceTable title="" :search-configs="searchConfigs" :data="tableData" :params="params" :columns="columns" :get-list="getData" :total="total" :loading="loading">
      <!-- <template v-slot:action>
        <router-link :to="{ name: 'ServiceVpcCreate', params: { type: 'CECSTACK' } }" class="m-l-xs">
...
export default {
  ...
  methods: {
    onmessage(data) {
      if (data.operate.indexOf('cecstack.vpc') > -1) {
        this.getData()
      }
    },
    getData(val) {
