  ...
  <div class="wrapper">
    ...
    <AdvanceTable title="" :search-configs="searchConfigs" :data="tableData" :params="params" :columns="columns" :get-list="getData" :total="total" :loading="loading">
      ...
      <template #operate="val, record">
        ...
        <el-button type="text" @click="addVpc(record)">挂载三层网络</el-button>
        <div class="action-divider"></div>
        <el-button type="text" @click="deleteVpc(record)">卸载三层网络</el-button>
      </template>
    </AdvanceTable>
    <add :add-data="addData" v-if="addData.dialog" @back="getData"></add>
    <AddVpcDialog :dialog="addVpcDialog" v-if="addVpcDialog.visible" @back="getData"></AddVpcDialog>
...
import AddVpcDialog from './dialog/AddVpcDialog.vue'
import { getGroup, removeGroup, detailGroup, getGroupRule, createGroupRule, removeGroupRule } from 'services/platform/index'
const columns = [
  {
    label: '组ID',
    prop: 'groupUuid',
  ...
  computed: {
    searchConfigs() {
      return [
        ...
        { type: 'Const', value: 'vendorType', initValue: 'ZSTACK' }
      ]
    }
  },
  components: { securityRule, add, AddVpcDialog },
  data() {
    return {
      columns,
      loading: false,
