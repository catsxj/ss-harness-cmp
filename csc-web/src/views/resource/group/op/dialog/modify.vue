<template>
  <div>
    <el-dialog title="编辑安全组" :visible.sync="addData.dialog" width="40%">
      <el-row>
...
<script>
/* global $ */
import { getRegion, modifyGroup, getVpc } from 'services/platform/index'
export default {
  props: {
    addData: {
      type: Object,
  ...
  methods: {
    ok() {
      this.$refs.data.validate(valid => {
        if (valid) {
          modifyGroup(this.addData.data).then(data => {
            if (data.success) {
              this.$message({
                type: 'success',
