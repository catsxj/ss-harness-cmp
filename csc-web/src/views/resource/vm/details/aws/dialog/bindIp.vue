            ...
            <el-col :span="24">
              <basic-form-item label="浮动IP" prop="id" validate="required" required-message="该字段为必填字段">
                <el-select v-model="addData.data.id">
                  <el-option v-for="item in floatIpList" :key="item.id" :label="item.name + '(' + item.publicIp + ')'" :value="item.id"></el-option>
                </el-select>
              </basic-form-item>
            </el-col>
          </el-row>
      ...
      this.$refs.bandIpData.validate(valid => {
        if (valid) {
          associateElasticipsIp(this.addData.data.id, {
            instanceId: this.addData.data.serverId,
