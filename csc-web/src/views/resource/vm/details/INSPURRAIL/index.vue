      ...
      <div class="detail-body-header" v-if="detail.showBtn">
        ...
        <el-row class="borders_btm" :gutter="10" style="margin-top: 2px">
          <el-col :span="24">
            ...
            <el-button type="ghost" v-if="detail.status === 'SUSPENDED'" :disabled="detail.isAlldisabled" size="medium" @click="patchVm('resume', '恢复')">恢复</el-button>
              ...
              <el-col :span="24">
                ...
                <el-card class="m-t" :body-style="{ padding: 0 }">
                  ...
                  <basic-table :data="networkConfigs" border style="margin: 10px">
                    ...
                    <el-table-column prop="networkName" label="所属网络"></el-table-column>
    ...
    upgradeSpec() {
      ...
      this.upgradeDialog = {
        ...
        data: {
          ...this.detail,
          flavorId: '', // 暂时不做回显
          service: 'inspurrail.standard.server',
          // 用来获取规格接口传参
          params: {},
          preview: [
    ...
    addDisk() {
      ...
      this.addDiskData = {
        ...
        data: {
          ...
          service: 'inspurrail.standard.server',
          params: {
            serviceCode: 'inspurrail.standard.volume',
    ...
    dilatation(data) {
      ...
      this.dilatationData = {
        ...
        data: {
          ...
          volumeId: data.id,
          service: 'inspurrail.standard.server',
          params: { osCategory: detail.osCategory, type: detail.vendorType, serviceCode: 'inspurrail.standard.volume' },
          preview: [
            { label: '云主机名称', value: name },
            { label: '云主机规格', value: `${cpu}核/${memory}GB/${disk}GB` },
            { label: 'IP地址', value: privateIpsList.map(item => item.address || item.ipAddress).join('、') || '--' }
