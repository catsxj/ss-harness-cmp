      ...
      <span>
        <el-form ref="data" :model="addData.data" :status-icon="true" label-width="120px">
          <el-form-item label="网卡：">
            <set-network :router-id="routerId" :network-list="ipList" ref="network" type="mount"></set-network>
          </el-form-item>
        </el-form>
      </span>
...
<script>
/* global $ */
import { conditionNetwork, vmAttachNetowrk } from 'services/platform/index'
...
export default {
  ...
  created() {
    conditionNetwork({
      condition: 'listRouterAndSubNet',
      vendorId: this.addData.data.vendorId,
      tenantUuid: this.addData.data.tenantUuid,
      serverId: this.addData.data.id
    }).then(data => {
      if (data.success) {
        this.ipList = data.data
        this.ipList.forEach(item => {
          if (item.routerUuid == this.addData.data.routerUuid) this.routerId = item.id
        })
      }
    })
  }
