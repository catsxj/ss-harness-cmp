      ...
      <div class="detail-body-header">
        ...
        <el-row class="borders_btm" :gutter="10" style="margin-top: 2px">
          <el-col :span="24">
            ...
            <el-button :disabled="detail.status === 'RUNNING' || (detail.disControl && detail.status != 'STOPPED') || detail.isExpired || detail.allDisabled" size="medium" @click="upgradeSpec">配置变更</el-button>
          </el-col>
        </el-row>
      ...
      <el-row class="detail-body-body" style="padding: 0 10px 0 10px">
        <el-tabs v-model="active">
              ...
              <el-col :span="24">
                <el-card :body-style="{ padding: 0 }" class="m-t">
                  ...
                  <el-table :data="networkConfigs" style="width: 100%">
                    <el-table-column label="网卡名称" show-overflow-tooltip>
                      <template slot-scope="scope"> nic{{ scope.$index }} </template>
                    </el-table-column>
                    <el-table-column prop="networkName" label="所属网络" show-overflow-tooltip></el-table-column>
                ...
                <iso ref="iso" :detail="detail" v-if="false">
                  <div class="pull-right" slot="operate">
                ...
                <snapshot ref="snapshot" :detail="detail">
          ...
          <el-tab-pane label="性能监控" name="monitor">
            <monitor :detail-id="detail.id" v-if="active === 'monitor'"></monitor>
          </el-tab-pane>
        </el-tabs>
      </el-row>
...
import monitor from '@/views/monitor/virtual/cloudTower/vm_detail.vue'
import headerContent from '../components/header.vue'
import Upgrade from '../components/upgradeSpec.vue'

import '../index.scss'
export default {
  components: { headerContent, netCard, Upgrade, AddDisk, mountDisk, mountIso, snapshot, volume, iso, detailMsg, res, addSnapshot, monitor },
  props: {
    detail: {
      type: Object,
      default: function () {
  ...
  methods: {
    setNetcards() {
      ...
      if (!this.detail.privateIpsList) return
    ...
    onmessage(data) {
      ...
      if (data.operate.indexOf('cloudtower.vm') > -1 && data.success && data.message.indexOf('转化为模板') > -1) {
        this.$emit('goBack')
      }
      // 虚拟卷操作成功
      if ((data.operate.indexOf('cloudtower.volume') > -1 || data.operate.indexOf('server.detachVolume') > -1) && data.success) {
        this.$refs.volume.getList()
      }
      // 快照操作成功
      if (data.operate.indexOf('cloudtower.snapshot') > -1 && data.success) {
        this.$refs.snapshot.getSnapshot(1)
      }
    },
    goBack() {
      this.$emit('goBack')
