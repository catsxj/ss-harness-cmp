  ...
  <div>
    <el-dialog append-to-body title="重设主机密码" width="40%" :close-on-click-modal="false" v-if="addData.dialog" :visible.sync="addData.dialog">
      <el-form :model="addData.data" :rules="rules" status-icon ref="data" label-width="100px" class="demo-ruleForm">
        <el-alert class="m-b-md" title="Linux机器密码需8到16位，Windows 机器密码需12到30位，至少包括三项（[a-z],[A-Z],[0-9]和[()`~!@#$%^&*-+=_|{}[]:;'<>,.?/]的特殊符号），且Windows不得包含用户名，如：“administrator”（大小写不敏感）。需要重启实例才能生效。" type="warning"> </el-alert>
        <el-form-item label="用户名：">
          <span v-if="addData.osCategory.indexOf('Windows') > -1">administrator ({{ addData.osCategory }})</span>
          <span v-else>root ({{ addData.osCategory }})</span>
        </el-form-item>
        <el-form-item label="新密码：" prop="newPasswd">
          <el-input type="password" v-model="addData.data.newPasswd" show-password></el-input>
        </el-form-item>
        <el-form-item label="确认密码：" prop="checkPass">
          <el-input type="password" v-model="addData.data.checkPass" show-password></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    var validatePass = (rule, value, callback) => {
      const reg = /^(?![a-zA-Z]+$)(?![A-Z0-9]+$)(?![A-Z`~!@#$%^&*()-+=|{}[\]/:;'<>,.?]+$)(?![a-z0-9]+$)(?![a-z`~!@#$%^&*()-+=|{}[\]/:;'<>,.?]+$)(?![0-9`~!@#$%^&*()-+=|{}[\]/:;'<>,.?]+$)[a-zA-Z0-9`~!@#$%^&*()-+=|{}[\]/:;'<>,.?]{8,30}$/
