<template>
  <div>
    <el-dialog title="卸载网卡" :visible.sync="addData.dialog" width="40%">
      <span>
        <el-form ref="unMountData" :model="addData.data" :status-icon="true" label-width="80px">
          <basic-form-item label="名称" prop="portId" validate="required" required-message="该字段为必填字段">
            <el-select v-model="addData.data.portId">
              <el-option v-for="item in unMountList" :key="item.id" :label="item.ipAddress" :value="item.id"></el-option>
            </el-select>
          </basic-form-item>
        </el-form>
      </span>
