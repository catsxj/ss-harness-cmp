  ...
  <el-form :label-width="labelWidth" :model="item" :disabled="disabled">
    <el-form-item :label="item.name + ':'" :title="item.name" prop="defaultValue" :rules="{ required: item.required, message: '该属性不能为空' }">
      ...
      <el-table v-else-if="item.type == 'TABLE'" :data="item.grid" empty-text="" border style="width: 100%">
        ...
        <div slot="empty">
          <el-button type="primary" size="mini" @click="tools.handleOperate({ flag: 3, item: item.grid })"><i class="iconfont icon-Added" />添加数据 </el-button>
        </div>
      </el-table>
      ...
      <el-input v-model="item.defaultValue" :type="showType" v-else-if="item.type === 'TEXTENCRYPT'">
...
<script>
import * as tools from './component/tools'
