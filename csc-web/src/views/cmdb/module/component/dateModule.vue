  ...
  <el-row>
    <el-popover ref="popover" placement="right" @show="tools.togglePopover(item)" @hide="tools.togglePopover(item)" trigger="click">
      <el-row class="field-options">
        ...
        <el-col :span="24" class="field-options-item">
          <label>默认值：</label>
          <div>
            <el-date-picker class="w-full" v-model="optionData.defaultValue" type="date" :disabled="optionData.reserved" value-format="yyyy-MM-dd" placeholder="选择日期"> </el-date-picker>
          </div>
        </el-col>
        <el-col :span="24" class="field-options-item">
          <label>必填：</label>
    ...
    <el-row v-popover:popover class="field-item" :class="{ selected: item.selected }">
      ...
      <el-col class="field-item-value">
        <el-date-picker class="w-full" v-model="optionData.defaultValue" type="date" value-format="yyyy-MM-dd" placeholder="选择日期"> </el-date-picker>
      </el-col>
