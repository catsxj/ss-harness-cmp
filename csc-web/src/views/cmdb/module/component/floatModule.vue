  ...
  <el-row>
    <el-popover ref="popover" placement="right" @show="tools.togglePopover(item)" @hide="tools.togglePopover(item)" trigger="click">
      <el-row class="field-options" :gutter="5">
        ...
        <el-col :span="24" class="field-options-item">
          <label class="required">保留几位小数：</label>
          <div>
            <el-input v-model="optionData.decimalNum" :disabled="optionData.reserved" @blur="validateDecimalNum(optionData.decimalNum)"></el-input>
          </div>
        </el-col>
        <el-col :span="12" class="field-options-item">
          <label>最小值：</label>
          <div>
            <el-input v-model="optionData.minValue" type="number" :disabled="optionData.reserved" @blur="tools.validateValue(1, optionData)"></el-input>
          </div>
        </el-col>
        <el-col :span="12" class="field-options-item">
          <label>最大值：</label>
          <div>
            <el-input v-model="optionData.maxValue" type="number" :disabled="optionData.reserved" @blur="tools.validateValue(2, optionData)"></el-input>
          </div>
        </el-col>
      </el-row>
    </el-popover>
...
export default {
  ...
  mounted() {},
  methods: {
    validateDecimalNum(value) {
      if (value) this.optionData.decimalNum = parseInt(Math.abs(value))
      else this.optionData.decimalNum = 2
      if (this.optionData.decimalNum > 10) this.optionData.decimalNum = 10
    }
  }
}
</script>
