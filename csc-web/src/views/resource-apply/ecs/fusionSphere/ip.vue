          ...
          <basic-table :data="list" :params="params" :get-list="getList" :total="total">
            <el-table-column show-overflow-tooltip label="IP" prop="ip">
              <template slot-scope="scope">
                <el-checkbox :label="scope.row.ip" :disabled="(checkList.length >= count && checkList.indexOf(scope.row.ip) == -1) || ip.indexOf(scope.row.ip) > -1">{{ scope.row.ip }}</el-checkbox>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    ok() {
      if (this.checkList.length == this.count || this.checkList.length == 0) {
