  ...
  <div>
    ...
    <basic-form-item label="数据盘：" class="m-b-lg">
      <el-row>
        ...
        <el-col :span="21" v-for="(cell, index) in disks" :key="index" class="m-t disk-cell">
          <!-- <el-col :span="2">
          ...
          <el-col :span="22">
            ...
            <el-col :span="24" class="m-t-xs" v-if="cell.createLvm">
              ...
              <el-autocomplete class="m-l-md w" v-if="cell.createLvm" v-model="cell.device" :fetch-suggestions="querySearch" placeholder="挂载路径"></el-autocomplete>
...
export default {
  ...
  methods: {
    ...
    // 获取服务类型
    getCategoryList() {
      getCategoriesByCode('smartx.standard.volume').then(data => {
        if (data.success) {
          if (!data.data.length) return this.$message.error('该服务不存在产品类型请检查')
          const obj = data.data.find(item => item.code === 'SMARTX-NORMAL')
