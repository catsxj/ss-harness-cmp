...
<template>
  <common-wrapper :add-data="addData" @vendorId="getList" ref="common" :elements="elements" :get-params="getParams" :loading="loading" vendorType="EASYSTACK" :item-data="retention" v-bind="$attrs">
...
export default {
  components: { CommonWrapper, ImageItem, network },
  mixins: [sku],
  props: {
    type: {
      type: String
  ...
  data() {
    return {
      elements: [
        {
          ...element,
          name: '云主机',
          serviceCode: 'easystack.standard.server',
          main: true
        },
        {
          ...element,
      ...
      addData: {
        ...
        service: 'easystack.standard.server',
        configs: {
          resourceLabel: [],
          diskCategoryId: '',
          osCategory: '',
  ...
  methods: {
    ...
    handleShowData() {
      ...
      const { name, password, keypairName, groups, login, region } = this.addData.configs
      this.vmData = {
        region,
        name: name,
      ...
      this.elements = [
        ...
        },
        {
          ...element,
          name: '系统盘',
          serviceCode: 'easystack.storage.volume',
          ...two
        }
      ]
      setTimeout(() => {
