...
<template>
  <el-form :label-width="labelWidth" :model="item" :disabled="disabled">
    <el-form-item :label="item.name + ':'" :title="item.name" prop="defaultValue" :rules="{ required: item.required, message: '该属性不能为空' }">
      ...
      <el-select filterable v-model="item.defaultValue" placeholder="请选择" v-else-if="item.type == 'SELECT'">
      ...
      <el-select filterable v-model="item.defaultValue" placeholder="请选择" v-else-if="item.type == 'QUOTE'">
