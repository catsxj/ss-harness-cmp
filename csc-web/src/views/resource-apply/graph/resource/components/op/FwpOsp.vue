<template>
  <basic-form :model="itemData" ref="form" :disabled="isPreview">
    ...
    <div v-if="!isTemplate">
      <RegionItem :add-data="itemData" @changeVendorId="getFirewall"></RegionItem>
      <h5>配置信息</h5>
      <div v-if="itemData.origin === 'create'">
        ...
        <el-form-item label="已审计：" prop="audited">
          <el-switch v-model="itemData.audited" :disabled="isPreview" active-color="#13ce66" inactive-color="#ff4949"> </el-switch>
        </el-form-item>
        <el-form-item label="描述：" prop="remark">
          <el-input type="textarea" v-model="itemData.remark" :disabled="isPreview"></el-input>
        </el-form-item>
      </div>
      <el-form-item v-else label="策略列表：" prop="id" :rules="{ required: true, message: '请选择列表' }">
...
import { getFirewallPolicy } from 'services/platform/index'
export default {
  mixins: [node, show],
