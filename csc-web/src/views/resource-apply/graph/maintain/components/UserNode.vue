<template>
  <basic-form :model="itemData" ref="form" :disabled="isPreview">
      ...
      <el-row>
        ...
        <el-col :span="24" v-if="osCategory == 'AIX'">
          <el-col :span="12">
            <el-form-item label="加密方式：" prop="crypted_type">
              <el-select filterable v-model="itemData.crypted_type" placeholder="加密方式">
        ...
        <el-col :span="12">
          <el-form-item label="过期时间：" prop="users.0.expires">
            <el-date-picker style="width: 100%" v-model="itemData.users[0].expires" type="date" placeholder="选择日期" format="yyyy/MM/dd" value-format="yyyy/MM/dd"> </el-date-picker>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="24" v-if="itemData.osCategory === 'Linux' && itemData.operate === 'add'">
          <el-form-item>
            <el-checkbox v-model="isSudo" @change="changeSudo">是否SUDO </el-checkbox>
          </el-form-item>
        </el-col>
      </el-row>
      <div v-if="isSudo">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="标签：" prop="sudo.tags">
              <el-select filterable clearable v-model="itemData.sudo.tags" placeholder="请选择标签">
    ...
    <el-row v-else-if="itemData.operate.indexOf('view') > -1">
      <el-row>
        <el-col :span="24">
          <el-form-item label="用户名称：" prop="sudo.name" :rules="{ pattern: /^[\w%\-+._]{1,}$/, message: '不能输入中文和除.+=_外的其他特殊字符', trigger: 'blur' }">
            <el-input class="width-90" v-model="itemData.sudo.name" placeholder="如果是一个用户组名称，请在名称前增加%。例如：%groupname"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="24">
          <el-form-item>
...
const typeData = [
  { name: '新增用户', value: 'add' },
  { name: '删除用户', value: 'delete' },
  { name: '修改用户', value: 'modify' },
  { name: '查看权限', value: 'view' },
  { name: '新增权限', value: 'addview' },
  { name: '删除权限', value: 'deleteview' }
]
// 非中文
const notChinese = {
