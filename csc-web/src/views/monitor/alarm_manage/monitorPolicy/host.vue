  ...
  <div>
    ...
    >
      <template #status="status">
        <status-icon :type="status | vmStatusColor">
          {{ status | openstackServer }}
        </status-icon>
      </template>
      <template #status2="status">
        <status-icon :type="status | xskyStatus('color')">
          {{ status | xskyStatus('status') }}
        </status-icon>
      </template>
      <template #catalog="catalog">
        {{ catalog | xskyHostType }}
      </template>
...
<script>
import { getHost } from 'services/platform/index'
