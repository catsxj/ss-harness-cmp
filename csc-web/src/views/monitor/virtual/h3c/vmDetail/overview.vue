...
export default {
    ...
    return {
      typeData: [
        ...
        },
        {
          title: '磁盘使用速率',
          value: 'disk',
          expr: JSON.stringify(['round(h3c_vm_partition_usage{name="' + this.identifier + '",id="' + this.instanceId + '"},0.01)']),
          name: '["磁盘使用速率"]',
          unit: 'KB/s'
        },
        {
          title: '内存利用率',
          value: 'mem',
          expr: JSON.stringify(['round(h3c_vm_mem_rate{name="' + this.identifier + '",id="' + this.instanceId + '"},0.01)']),
          name: '["内存利用率"]',
          unit: '%'
        }
      ],
  ...
  methods: {
    ...
    getDashboard() {
      console.log(JSON.parse(JSON.stringify(this.instanceId)))
      const expr1 = 'round(h3c_vm_cpu_rate{name="' + this.identifier + '",id="' + this.instanceId + '"},0.01)'
      const expr2 = 'round(h3c_vm_mem_rate{name="' + this.identifier + '",id="' + this.instanceId + '"},0.01)'
      getHostDashboard({
        expr: expr1,
        simple: true
      }).then((data) => {
