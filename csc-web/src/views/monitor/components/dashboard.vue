<template>
  <div>
    ...
    <el-row :gutter="5">
      <el-col :span="24 / resUsedData.length" v-for="item in resUsedData" :key="item.name" class="m-t-xs">
      ...
      <el-col :span="24" class="m-b-xs">
        <el-card class="box-card">
          ...
          <div>
            <line-charts ref="chart2" height="250px" width="100%" id="cpu" :data="chart2Data" :setting="setting2" v-if="chart2Data"></line-charts>
          </div>
        </el-card>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    getDetail() {
      detailCloudVendor(this.vendorId).then(data => {
        if (data.success) {
      ...
      getPortal({
        ...
        if (data.success) {
          this.resUsedData = data.data.filter(item => {
            if (['ARCHEROS', 'CLOUDTOWER', 'SANGFOR'].includes(this.vendorType)) {
