    ...
    <el-row type="flex" justify="space-between">
      <el-col :span="Math.ceil(24 / list.length)" class="row_margin box" v-for="(item, index) in list" :key="index">
        ...
        <el-col :span="16" class="box_value">
          ...
          <div v-else-if="item.type == 'cecstack' || item.type == 'cnware' || item.type == 'h3c'">
            <div v-for="(obj, index) in detail[item.value]" :title="obj.address" :key="index">(内网){{ !obj.address ? '' : obj.address + (!obj.networkName ? '' : '(' + obj.networkName + ')') }}</div>
            <div v-if="item.value2">
              <div v-for="(obj, index) in detail[item.value2]" :title="obj.address" :key="index">(公网){{ obj.address }}</div>
            </div>
          </div>
