<template>
  <el-card class="wrapper">
    <common-detail v-if="detailFlag" :setting="{ type: 'Etcd' }" :title="detail.name" @goBack="goBack">
...
const searchConfigs = [
  ...
  { type: 'Const', value: 'type', initValue: 'Etcd' },
  ...
  methods: {
    ...
    alarmDetail(row) {
      this.$router.push({
        ...
        query: { id: row.id, name: row.name, type: 'MONITOR_CCS_ETCD' }
      })
    },
    handleLink(id) {
      this.ruleGroupIds = []
