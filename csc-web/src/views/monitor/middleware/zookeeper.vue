<template>
  <el-card class="wrapper">
    ...
    <div>
      <table-search :configs="searchConfigs" :onSearch="handleSearch"> </table-search>
      <basic-table :data="list" :params="params" :get-list="getList" :total="total" @select="handleSelectItem" @select-all="handleSelectAll" ref="databaseTable">
        ...
        <el-table-column label="所属租户" show-overflow-tooltip prop="tenantName"> </el-table-column>
          ...
          <template slot-scope="scope">
            ...
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item @click.native="handleLink(scope.row, 1)"> 关联阈值组 </el-dropdown-item>
                <el-dropdown-item @click.native="handleLink(scope.row, 2)"> 关联分发策略 </el-dropdown-item>
                <el-dropdown-item v-if="scope.row.thresholdGroupName" @click.native="handleUnLink(scope.row.ruleRelationId, 1)"> 取消关联阈值组 </el-dropdown-item>
                <el-dropdown-item v-if="scope.row.policyName" @click.native="handleUnLink(scope.row.ruleRelationId, 2)"> 取消关联分发策略 </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    alarmDetail(row) {
      this.$router.push({
        ...
        query: { id: row.id, name: row.name, type: 'MONITOR_CCS_ZOOKEEPER' }
      })
    },
    handleLink(id) {
      this.ruleGroupIds = []
