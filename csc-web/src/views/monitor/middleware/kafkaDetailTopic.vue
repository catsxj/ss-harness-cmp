      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="topic：">
            <el-select v-model="topic" placeholder="请选择" @change="changeTopic">
              <el-option v-for="item in topicsData" :key="item.name" :label="item.name" :value="item.name"> </el-option>
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>
...
import { getCmsTopics } from 'services/monitor/middleware'
const intervalData = [
  { name: '1H', value: '1H' },
  { name: '6H', value: '6H' },
  { name: '1D', value: '1D' },
...
export default {
  ...
  methods: {
    getTopicsData() {
      getCmsTopics(this.detailId).then(data => {
        if (data.success) {
          this.topicsData = data.data
          if (this.topicsData.length) {
            this.topic = this.topicsData[0].name
    ...
    getFirstChart() {
      const expr = JSON.stringify(['round(sum(irate(kafka_server_brokertopicmetrics_messagesin_total{cms_id="' + this.detailId + '",topic="' + this.topic + '"}[5m])),0.01)'])
      getCharts({
    ...
    getSecondChart() {
      const str1 = 'round(sum(1/1024*irate(kafka_server_brokertopicmetrics_bytesin_total{cms_id="' + this.detailId + '",topic="' + this.topic + '"}[5m])),0.01)'
      const str2 = 'round(sum(1/1024*irate(kafka_server_brokertopicmetrics_bytesout_total{cms_id="' + this.detailId + '",topic="' + this.topic + '"}[5m])),0.01)'
      const str3 = 'round(sum(1/1024*irate(kafka_server_brokertopicmetrics_bytesrejected_total{cms_id="' + this.detailId + '",topic="' + this.topic + '"}[5m])),0.01)'
