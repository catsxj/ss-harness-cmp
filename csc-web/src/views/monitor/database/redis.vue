...
export default {
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_KVS_REDIS'
    }
  },
  created() {
    this.handleSearch()
  ...
  methods: {
    ...
    alarmDetail(row) {
      this.$router.push({
        ...
        query: { id: row.id, name: row.name, type: 'MONITOR_KVS_REDIS' }
      })
    },
    handleLink(id) {
      this.ruleGroupIds = []
    ...
    getDetail(id) {
      this.$router.push({ name: 'NosqlRedisDetail', query: { id: id } })
    }
  }
}
</script>
