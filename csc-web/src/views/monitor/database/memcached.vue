...
const searchConfigs = [
  ...
  { type: 'Const', value: 'type', initValue: 'MemCache' },
...
export default {
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_KVS_MEMCACHED',
      detailFlag: false,
      loading: false
    }
  },
  ...
  methods: {
    ...
    alarmDetail(row) {
      this.$router.push({
        ...
        query: { id: row.id, name: row.name, type: 'MONITOR_KVS_MEMCACHED' }
      })
    },
    handleLink(id) {
      this.ruleGroupIds = []
    ...
    getDetail(id) {
      this.$router.push({ name: 'NosqlMemcachedDetail', query: { id: id } })
    }
  }
}
</script>
