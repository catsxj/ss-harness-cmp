...
export default {
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_RDS_SQLSERVER'
    }
  },
  ...
  methods: {
    ...
    alarmDetail(row) {
      this.$router.push({
        ...
        query: { id: row.id, name: row.name, type: 'MONITOR_RDS_SQLSERVER' }
      })
    },
    handleLink(id) {
      this.ruleGroupIds = []
    ...
    getDetail(id) {
      this.$router.push({ name: 'RdbmsMssqlDetail', query: { id: id } })
    }
  }
}
</script>
