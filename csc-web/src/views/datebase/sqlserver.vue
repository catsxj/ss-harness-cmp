    ...
    <div>
      <table-search class="m-b" :configs="searchConfigs" :onSearch="handleSearch">
        <template v-slot:operate>
          <router-link :to="{ name: 'ServiceSqlServerCreate', params: { code: 'rds.sqlserver' } }">
      ...
      <basic-table :data="list" :params="params" :get-list="getList" :total="total">
        ...
        <el-table-column label="操作" width="220px">
          <template slot-scope="scope">
            ...
            <modifySpec :detail="scope.row" type="sqlserver" tip="扩容期间可能会重启数据库多次，请您再三确认" :closable="false"></modifySpec>
          </template>
        </el-table-column>
      </basic-table>
    </div>
...
import user from './sqlServerUser/index'
...
const searchConfigs = [
  ...
  { type: 'Const', value: 'type', initValue: 'SQLServer' },
...
export default {
  ...
  created() {
    this.detailSetting.image = this.serviceIcon['rds.sqlserver']
  },
  methods: {
    ...
    onmessage(data) {
      if (data.operate.indexOf('volume') > -1 || data.operate.indexOf('sqlserver') > -1) {
        this.getList()
      }
