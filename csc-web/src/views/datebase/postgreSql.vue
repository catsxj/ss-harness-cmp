  ...
  <el-card class="wrapper">
    <common-detail v-if="detailFlag" :title="detail.name" @goBack="goBack">
      ...
      <el-tabs v-model="active">
        ...
        <el-tab-pane label="访问地址" name="connection">
    ...
    <div>
      <table-search class="m-b" :configs="searchConfigs" :onSearch="handleSearch">
        <template v-slot:operate>
          <router-link :to="{ name: 'ServicePgCreate', params: { code: 'rds.postgresql' } }">
      ...
      <basic-table :data="list" :params="params" :get-list="getList" :total="total">
        ...
        <el-table-column label="操作" width="220px">
          <template slot-scope="scope">
            ...
            <modifySpec :detail="scope.row" service="rds.postgresql"></modifySpec>
          </template>
        </el-table-column>
      </basic-table>
    </div>
...
import dbs from './postgreSqlDbs/index.vue'
import user from './postgreSqlUser/index.vue'
...
export default {
  components: { modifySpec, dbs, user, copy, join, log, argument, UnsubscribeService, DelayService, DbConnect },
  mixins: [webSocket],
  data() {
    return {
      searchConfigs,
  ...
  created() {
    this.detailSetting.image = this.serviceIcon['rds.postgresql']
  },
  methods: {
    ...
    onmessage(data) {
      if (data.operate.indexOf('volume') > -1 || data.operate.indexOf('postgresql') > -1) {
        this.getList()
      }
