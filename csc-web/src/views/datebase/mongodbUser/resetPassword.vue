<template>
  <div>
    <el-dialog title="重置密码" :visible.sync="addData.dialog" width="50%" append-to-body>
      <el-row>
        <el-form label-width="120px" :model="addData.data" ref="data">
          ...
          <el-col :span="24">
            <basic-form-item label="新密码:" prop="newPassword" validate="required,datebasePasswords">
              <el-input type="password" v-model="addData.data.newPassword"></el-input>
            </basic-form-item>
          </el-col>
          <el-col :span="24">
            <basic-form-item label="确认密码:" prop="endPassword" validate="required,datebasePasswords">
...
<script>
/* global $ */
import { patchKvsUsers } from 'services/resource/datebase'
    ...
    ok() {
      this.$refs.data.validate(valid => {
        if (valid) {
          ...
          patchKvsUsers('reset', obj).then(data => {
            if (data.success) {
              this.$message({
                type: 'success',
