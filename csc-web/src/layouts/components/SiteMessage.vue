<template>
  <div class="system-letter">
    <!-- <span>
      <router-link to="/shop" class="icon m-r-sm">
        <el-badge :value="totalShop" class="badge-item" :hidden="totalShop === 0">
          <svg-icon icon-name="svg-shop-cart"></svg-icon>
        </el-badge>
      </router-link>
    </span> -->
    <el-dropdown>
      <el-badge :value="totalMessage" class="item cur-point" :hidden="totalMessage === 0">
        <i class="el-icon-message" style="font-size: 18px; color: #dcdce0"></i>
      </el-badge>
...
import { getShoppingCart } from 'services/system/shop_cart'
...
export default {
  ...
  methods: {
    ...
    async getShoppingCart() {
      const data = await getShoppingCart({ page: 1, rows: 5 })
      if (data.success) {
        this.totalShop = data.data.total
      }
    },
    ...
    goProfileMessage(id) {
      if (this.$route.name !== 'MyMessage') this.$router.push({ name: 'MyMessage' })
      this.$store.commit('SET_PROFILEMESSAGEId', id)
    }
  }
}
</script>
