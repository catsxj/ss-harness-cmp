<template>
  <div class="wrapper">
    <div v-if="!detailFlag">
      <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="columns" :get-list="getList" :total="total" :loading="loading" @select="handleSelectItem" @select-all="handleSelectAll" ref="manageMonitorTable">
  ...
  data() {
    return {
      ...
      searchConfigs: [
        ...
        { type: 'Const', value: 'type', initValue: 'MGMT' }
      ],
      columns,
      loading: false,
      params: {
      ...
      getPort(this.params).then((data) => {
        ...
        if (data.success) {
          ...
          this.list.forEach((item) => {
            const self = this
            setTimeout(() => {
              if (self.idList.indexOf(item.id) > -1) self.$refs.manageMonitorTable.toggleRowSelection(item, true)
            })
          })
        }
      })
