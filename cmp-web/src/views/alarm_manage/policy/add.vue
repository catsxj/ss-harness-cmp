<template>
  <div>
    <basic-form :model="addData" label-width="110px" ref="addData">
      <el-row class="search-container m-b-md">
        ...
        <el-col :span="24">
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              <basic-form-item label="延迟时间：" prop="waitTime" title="指告警发生后如果未恢复正常，时隔多久触发该分发策略" validate="required" required-message="请选择延迟时间">
                <el-select clearable v-model="addData.waitTime" placeholder="延迟时间">
                  <el-option v-for="(item, index) in waitTimeArr" :key="index" :label="waiteTimeFilter(item)" :value="item"> </el-option>
                </el-select>
              </basic-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="24">
              <el-col :span="12">
                <basic-form-item label="VC原生告警：" prop="nativeAlarm">
                  <el-switch v-model="addData.nativeAlarm" active-color="#13ce66" inactive-color="#ff4949"> </el-switch>
                </basic-form-item>
              </el-col>
            </el-col>
          </el-row>
      ...
      <add-user class="m-b" ref="receiveUser" :receiverIds="addData.receiverIds" :flag="flag" :addData="addData"></add-user>
      <add-role class="m-b" ref="receiveRole" :roleIds="addData.roleIds" :flag="flag"></add-role>
...
<script>
import { getDistriDetail, createDistri, modifyDistri, removeDistri } from 'services/monitor'
import { waiteTimeFilter } from '@/filters/index'
    ...
    addSubmit() {
      this.$refs.addData.validate((valid) => {
        if (valid) {
          ...
          const roleIds = JSON.stringify(this.$refs.receiveRole.ids)
          addData.receiverIds = receiverIds
          addData.sendTargetOwner = this.$refs.receiveUser.formData.sendTargetOwner
          addData.roleIds = roleIds
          if (addData.receiverIds.length == 0 && addData.roleIds.length == 0) {
          ...
          if (this.status == 'create') {
            ...
            delete addData.stringBlockPolicy
