...
import { getSystem } from 'services/platform/xsky'
import MyProgress from 'views/storage/distribute/xsky/components/progress'
import { statusFilter, stautsIconFilter, capacityFilter } from 'views/storage/distribute/xsky/filters'
export default {
  ...
  data() {
    return {
      columns: [
        ...
        {
          label: '容量使用',
          scopedSlots: { customRender: 'sizeUsed' }
        },
        {
          label: '所属存储池',
          prop: 'poolName'
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      this.refreshId()
      getSystem(this.params).then((data) => {
        this.loading = false
        if (data.success) {
