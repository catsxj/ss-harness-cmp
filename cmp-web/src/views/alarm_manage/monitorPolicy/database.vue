<template>
  <div>
    <AdvanceTable :title="`${type}列表`" :search-configs="searchConfigs" :data="list" :params="params" :columns="columnsType ? columns : columns2" :get-list="getList" :total="total" ref="ResourceTable" @select="handleSelectItem" @select-all="handleSelectAll" :loading="loading" :isInitSearch="false">
...
export default {
  ...
  created() {
    console.log(this.resourceIds === null || this.resourceIds.length)
    switch (this.type) {
      case 'mysql':
      case 'sqlserver':
      case 'postgresql':
