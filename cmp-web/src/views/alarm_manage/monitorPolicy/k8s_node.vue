  ...
  <div>
    ...
    >
      <template #schedulable="schedulable">
        {{ booleanFilter(schedulable) }}
      </template>
      <template #setting="val, record">
        {{ record.cpu + '核/' + record.memory + 'GB' }}
      </template>
      <template #cpuUsed="val, record">
        {{ record.cpuUsed + '(' + record.cpuPercent + '%)' }}
      </template>
      <template #memoryUsed="val, record">
        {{ record.memoryUsed + '(' + record.memoryPercent + '%)' }}
      </template>
    </AdvanceTable>
  </div>
</template>
...
import { getNode } from 'services/platform/k8s'
export default {
  props: {
    resourceType: {
      type: String
