<template>
  <div>
    <common-detail v-if="detailFlag" :setting="{ type: 'Kafka' }" :title="detail.name" @goBack="goBack">
...
import detail from './kafkaDetail'
...
export default {
  ...
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_CMS_KAFKA'
    }
  },
  computed: {
    userData() {
  ...
  methods: {
    onmessage(data) {
      if (data.operate.indexOf('kafka') > -1) {
        this.getList()
      }
    },
    goBack() {
