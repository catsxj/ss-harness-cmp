        ...
        <div v-if="!detailFlag">
          <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="columns" :get-list="getList" :total="total" :loading="loading" @select="handleSelectItem" @select-all="handleSelectAll" ref="resourceMonitorTable">
            ...
            <template #privateIps="privateIps, record">
              <div v-for="item in record.privateIps" :key="item.address">(内网){{ item.address }}</div>
              <div v-for="item in record.publicIps" :key="item.address">(内网){{ item.address }}</div>
            </template>
...
export default {
  ...
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_CECSTACK_VM'
    }
  },
  methods: {
    goBackCloud() {
