    ...
    return {
      typeData: [
        { title: '磁盘使用速率', value: 'usage', expr: JSON.stringify(['round(vsphere_host_disk_usage_average{esxhostname="' + this.detailName + '"},0.01)']), name: "['磁盘使用速率']", unit: 'KB/s' },
        {
          title: '磁盘读取/写入速率',
          value: 'disk',
          expr: JSON.stringify(['vsphere_host_disk_read_average{disk="instance-total",esxhostname="' + this.detailName + '"}', 'vsphere_host_disk_write_average{disk="instance-total",esxhostname="' + this.detailName + '"}']),
          name: "['磁盘读取速率', '磁盘写入速率']",
          unit: 'KB/s'
        },
