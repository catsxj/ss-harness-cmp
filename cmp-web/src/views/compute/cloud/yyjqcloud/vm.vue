  ...
  <div>
    <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="columns" :get-list="getList" :total="total" :loading="loading" @select="handleSelectItem" @select-all="handleSelectAll" ref="resourceMonitorTable">
      ...
      <template #privateIps="privateIps, record">
        <div v-for="item in record.privateIps" :key="item">(内网){{ item.address }}</div>
      ...
      <template #spec="val, record">
        <div v-if="record.vendorType == 'YYJQCLOUD' || record.vendorType == 'QCLOUD'">{{ (record.cpu ? record.cpu : 0) + 'C/' + (record.memory ? record.memory : 0) + 'GB/' }}{{ (record.disk == null ? 0 : record.disk) + (record.dataDiskSize == null ? 0 : record.dataDiskSize) + 'GB' }}</div>
        <div v-else>{{ (record.cpu ? record.cpu : 0) + 'C/' + (record.memory ? record.memory : 0) + 'GB/' }}{{ record.disk == null ? 0 : record.disk + 'GB' }}</div>
      </template>
    ...
    <common-detail v-if="detailFlag" :title="detailData.name" @goBack="goBack">
      <div slot="custom_content">
        <detail :detail-id="detailId" :instance-id="instanceId" :identifier="identifier" :host-name="hostName" :bios-uuid="biosUuid"></detail>
      </div>
    </common-detail>
  </div>
</template>
...
<script>
import { statusOnColorFilter, vmStatusFilter, vmStatusColorFilter, openstackServerFilter } from '@/filters/index'
  ...
  data() {
    return {
      ...
      searchConfigs: [
        ...
        { type: 'Const', value: 'vendorType', initValue: 'YYJQCLOUD' },
      ...
      biosUuid: '',
      instanceId: '',
      resourceType: 'MONITOR_QCLOUD_VM'
    }
  },
  methods: {
    getList() {
