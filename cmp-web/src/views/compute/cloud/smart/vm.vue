<template>
  <div>
    ...
    <add-policy v-if="addFlag" resourceType="MONITOR_SMARTX_VM" @cancle="cancle" :add-link-flag="addFlag" :resourceId="resourceId" :ruleGroupIds="ruleGroupIds"></add-policy>
    ...
    <common-detail v-if="detailFlag" :title="detailData.name" @goBack="goBack">
      <div slot="custom_content">
        <detail :detail-id="detailId" :instanceId="instanceId"></detail>
      </div>
    </common-detail>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      searchConfigs: [
        ...
        { type: 'Const', value: 'vendorType', initValue: 'SMARTX' },
  ...
  methods: {
    ...
    handlePolicy(data) {
      this.handleStart(this, data, 'MONITOR_SMARTX_VM')
    },
    getDetail(row) {
    ...
    handleMonitorDetail(id, name) {
      this.$router.push({
        ...
        query: { id: id, name: name, type: 'MONITOR_SMARTX_VM' }
      })
    },
    setIP(row) {
      this.setData = {
