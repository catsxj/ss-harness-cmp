<template>
  <div>
    ...
    <add-policy v-if="addFlag" resourceType="MONITOR_SANGFOR_HCI_VM" @cancle="cancle" :add-link-flag="addFlag" :resourceId="resourceId" :ruleGroupIds="ruleGroupIds"></add-policy>
...
export default {
  ...
  methods: {
    ...
    handlePolicy(data) {
      this.handleStart(this, data, 'MONITOR_SANGFOR_HCI_VM')
    },
    getDetail(row) {
    ...
    handleMonitorDetail(id, name) {
      this.$router.push({
        ...
        query: { id: id, name: name, type: 'MONITOR_SANGFOR_HCI_VM' }
      })
    },
    setIP(row) {
      this.setData = {
