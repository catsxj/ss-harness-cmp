...
<!--        <el-progress :stroke-width="16" :color="customColorMethod"-->
<!--          :percentage="(val && record.config.cpuHz) ? Number(((val /record.config.cpuHz) *100).toFixed(2)) : 0"></el-progress>-->
      </template>
    ...
    <common-detail v-if="detailFlag" :title="detailData.name" @goBack="goBack">
      <div slot="custom_content">
        <detail :detail-id="detailId" :detail-name="detailData.name" :detail="detail"></detail>
      </div>
    </common-detail>
  </div>
</template>
  ...
  data () {
    return {
      ...
      searchConfigs: [
        ...
        // { type: 'Const', value: 'catalog', initValue: 'FUSIONSPHERE' },
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_VCENTER_HOST',
      detail: {}
    }
  },
  methods: {
