  ...
  <div>
    <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="vmColumns" :get-list="getList" :total="total" :loading="loading" @select="handleSelectItem" @select-all="handleSelectAll" ref="resourceMonitorTable">
      ...
      <template #status="status">
        <status-icon :type="vmStatusColorFilter(status)">{{vmStatusFilter(status)}}
        </status-icon>
      </template>
      <template #cpuUsage="cpuUsage">
        <el-progress class="progress" :color="customColorMethod"  :stroke-width="16" :percentage="cpuUsage?cpuUsage:0"></el-progress>
      </template>
      <template #memUsage="memUsage">
        <el-progress class="progress" :color="customColorMethod"  :stroke-width="16" :percentage="memUsage?memUsage:0"></el-progress>
      </template>
      ...
      <template #operate="val, record">
        ...
        <el-dropdown trigger="click">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item @click="handleLink(record.id)">
              关联规则
            </el-dropdown-item>
            <el-dropdown-item @click="handlePolicy(record)">
              {{`${record.alarmEnable ? '屏蔽':'开启'}告警`}}
            </el-dropdown-item>
            <el-dropdown-item @click="setIP(record)">
              设置监控IP
            </el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
    ...
    <common-detail v-if="detailFlag" :title="detailData.name" @goBack="goBack">
      <div slot="custom_content">
        <detail :detail-id="detailId" :identifier="identifier" :host-name="hostName" :bios-uuid="biosUuid" :detail="detail"></detail>
      </div>
    </common-detail>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      searchConfigs: [
        ...
        { type: 'Select', label: '状态', value: 'status', data: [{ id: 'RUNNING', name: '运行中' }, { id: 'BUILDING', name: '创建中' }, { id: 'STOPPED', name: '关机' }, { id: 'SUSPENDED', name: '挂起' }, { id: 'EXCEPTION', name: '异常' }, { id: 'UNKNOWN', name: '断开' }] },
  ...
  methods: {
    ...
    handlePolicy (data) {
      ...
      this.handleStart(this, data, 'MONITOR_FUSIONSPHERE_VM')
    },
    getDetail (row) {
    ...
    handleMonitorDetail (id, name) {
      this.$router.push({
        ...
        query: { id: id, name: name, type: 'MONITOR_FUSIONSPHERE_VM' }
      })
    },
    setIP (row) {
      this.setData = {
