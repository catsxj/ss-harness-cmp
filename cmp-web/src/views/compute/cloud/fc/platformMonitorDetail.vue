    ...
    <div>
      ...
      <el-row class="m-t-xs" :gutter="5">
        <el-col :span="10">
          ...
          <pie-charts ref="chart1" v-if="chart1Data" id="chart1" :data="chart1Data" width="100%" :setting="{color: ['#EC1C24', '#FF7F27', '#FFF200', '#4EAFF5']}"
                      height="250px" theme="告警统计"></pie-charts>
          </el-card>
        </el-col>
        <el-col :span="14">
          <el-card class="box-card">
            <div slot="header" class="clearfix">
              ...
              <span class="pull-right">
                <el-button-group>
                  <el-button size="mini" :type="item.value == chart2.step?'primary':''" v-for="item in intervalData"
                             :key="item.value"
                             @click="changeInterval(item.value)">{{item.name}}</el-button>
                  <el-button size="mini" :type="chart2.step === 0?'primary':''" @click="userDefind()">自定义</el-button>
                </el-button-group>
              </span>
            </div>
            <div>
              <bar-charts ref="chart2" height="250px" width="100%" id="cpu" :data="chart2Data" :setting="{color: ['#EC1C24', '#FF7F27', '#FFF200', '#4EAFF5']}"
                          v-if="chart2Data"></bar-charts>
            </div>
          </el-card>
        </el-col>
      </el-row>
