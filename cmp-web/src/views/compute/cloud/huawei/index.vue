        ...
        <div v-if="!detailFlag">
          <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="columns" :get-list="getList" :total="total" :loading="loading" @select="handleSelectItem" @select-all="handleSelectAll" ref="resourceMonitorTable">
            ...
            <template #privateIps="privateIps, record">
              <div v-for="item in privateIps" :key="item.address">{{ item.type == 'floating' ? '(公网)' : '(内网)' }}{{ !item.address ? '' : item.address + (!item.networkName ? '' : '(' + item.networkName + ')') }}</div>
...
export default {
  ...
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_HUAWEI_VM'
    }
  },
  methods: {
    goBackCloud() {
