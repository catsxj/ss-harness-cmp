...
export default {
      ...
      typeData: [
        {
          title: 'CPU利用率',
          value: 'cpu',
          expr: JSON.stringify(['round(bocloud_inspurRail_host_CPU{id="' + this.detailId + '"}, 0.01)']),
          name: '["cpu利用率"]',
          unit: '%'
        },
        {
          title: '磁盘使用速率',
          value: 'disk',
          expr: JSON.stringify(['round(bocloud_inspurRail_host_STORAGE{id="' + this.detailId + '"},0.01)']),
          name: "['磁盘使用速率']",
          unit: 'KB/s'
        },
        {
          title: '内存利用率',
          value: 'mem',
          expr: JSON.stringify(['round(bocloud_inspurRail_host_MEM{id="' + this.detailId + '"}, 0.01)']),
          name: "['内存利用率']",
          unit: '%'
        }
      ],
  ...
  methods: {
    getDetail() {
      getHostDetail(this.detailId, 'INSPURRAIL').then((data) => {
        if (data.success) {
    ...
    getDashboard() {
      const expr1 = 'round(bocloud_inspurRail_host_CPU{id="' + this.detailId + '"},0.01)'
      const expr2 = 'round(bocloud_inspurRail_host_MEM{id="' + this.detailId + '"},0.01)'
      getHostDashboard({
        expr: expr1,
        simple: true
      }).then((data) => {
