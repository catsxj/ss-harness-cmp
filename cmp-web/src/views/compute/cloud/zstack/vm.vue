<template>
  <div>
    ...
    <add-policy v-if="addFlag" resourceType="MONITOR_ZSTACK_VM" @cancle="cancle" :add-link-flag="addFlag" :resourceId="resourceId" :ruleGroupIds="ruleGroupIds"></add-policy>
    ...
    <common-detail v-if="detailFlag" :title="detailData.name" @goBack="goBack">
      <div slot="custom_content">
        <detail :detail-id="detailId" :identifier="identifier" :host-name="hostName" :bios-uuid="biosUuid" :instance-id="instanceId"></detail>
      </div>
    </common-detail>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handlePolicy(data) {
      this.handleStart(this, data, 'MONITOR_ZSTACK_VM')
    },
    getDetail(row) {
    ...
    handleMonitorDetail(id, name) {
      this.$router.push({
        ...
        query: { id: id, name: name, type: 'MONITOR_ZSTACK_VM' }
      })
    },
    setIP(row) {
      this.setData = {
