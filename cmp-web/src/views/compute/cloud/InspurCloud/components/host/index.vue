  ...
  <div>
    <AdvanceTable title="" :search-configs="searchConfigs" :data="list" :params="params" :columns="columns" :get-list="getList" :total="total" :loading="loading" @select="handleSelectItem" @select-all="handleSelectAll" ref="resourceMonitorTable">
      ...
      <template #cpuUsage="cpuUsage">
        <el-progress class="progress" :color="customColorMethod" text-inside :stroke-width="16" :percentage="cpuUsage ? cpuUsage * 1 : 0"></el-progress>
      </template>
      <template #memUsage="memUsage">
        <el-progress class="progress" :color="customColorMethod" text-inside :stroke-width="16" :percentage="memUsage ? memUsage * 1 : 0"></el-progress>
      </template>
...
import { getFusionHost, getFusionHostDetail, getRuleGroupBind, exportFusionHost } from 'services/monitor/index.js'
...
export default {
  ...
  data() {
    return {
      ...
      resourceId: 0,
      ruleGroupIds: [],
      resourceType: 'MONITOR_INSPURCLOUD_HOST'
    }
  },
  methods: {
    // 导出数据
    exportData() {
      exportFusionHost(this.params)
    },
    getList() {
      this.refreshId()
